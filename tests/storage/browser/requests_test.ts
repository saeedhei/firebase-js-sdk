...
describe("Firebase Storage > Requests", () => {
  ...
  const metadataFromServerResource =
      fromResourceString(authWrapper, serverResourceString, mappings);
  ...
  function assertBodyEquals(body: Blob|string|Uint8Array|null,
                            expectedStr: string): Promise<void> {
    if (body === null) {
  ...
  it("updateMetadata handler", () => {
    const requestInfo = requests.updateMetadata(authWrapper, locationNormal,
                                                metadata, mappings);
    checkMetadataHandler(requestInfo);
  });
  ...
  it("multipartUpload request info", () => {
    ...
    for (let i = 0; i < maps.length; i++) {
      ...
      const matches = (requestInfo.headers['Content-Type'] as string)
                          .match(multipartHeaderRegex);
      assert.isNotNull(matches);
      assert.equal(matches.length, 2);
      const boundary = matches[1];
      promises.push(assertBodyEquals(requestInfo.body,
                                     makeMultipartBodyString(boundary)));
  ...
  it("multipartUpload handler", () => {
    const requestInfo = requests.multipartUpload(authWrapper, locationNormal,
                                                 mappings, smallBlob, metadata);
    checkMetadataHandler(requestInfo);
  });
  ...
  function testCreateResumableUploadHandler() {
    ...
    const handlerUrl = requestInfo.handler(fakeXhrIo({
  ...
  describe("getResumableUploadStatus handler", () => {
    ...
    let status = requestInfo.handler(fakeXhrIo({
                                       'X-Goog-Upload-Status' : 'active',
    ...
    status = requestInfo.handler(
        ...
        '');
    expectedStatus = new requests.ResumableUploadStatus(smallBlob.size(),
                                                        smallBlob.size(), true);
    assert.deepEqual(status, expectedStatus);
  });
  ...
  it("continueResumableUpload handler", () => {
    ...
    let status = requestInfo.handler(
        fakeXhrIo({'X-Goog-Upload-Status' : 'final'}), serverResourceString);
    ...
    status =
        requestInfo.handler(fakeXhrIo({'X-Goog-Upload-Status' : 'active'}), '');
    expectedStatus =
