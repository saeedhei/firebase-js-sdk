...
function makeStorage(url: string) {
  function maker(wrapper, loc) { return ({} as any) as Reference; }
  const authWrapper =
      new AuthWrapper(null, maker, makeRequest, ({} as any) as Service,
...
describe("Firebase Storage > Reference", () => {
  ...
  describe("Path constructor", () => {
    it("root", () => { assert.equal(root.toString(), 'gs://test-bucket/'); });
  ...
  describe("parent", () => {
    it("Returns null at root", () => { assert.isNull(root.parent); });
  ...
  describe("child", () => {
    ...
    it("compresses repeated slashes", () => {
      assert.equal(root.child('//a///b/////').toString(),
                   'gs://test-bucket/a/b');
    });
  ...
  it("Works if the user logs in before creating the storage reference",
     (done) => {
  ...
  describe("putString", () => {
    ...
    it("Uses embedded content type in DATA_URL format", () => {
      const task =
          child.putString('data:lol/wat;base64,aaaa', StringFormat.DATA_URL);
    ...
    it("Lets metadata.contentType override embedded content type in DATA_URL format",
       () => {
         const task = child.putString(
             'data:ignore/me;base64,aaaa', StringFormat.DATA_URL,
  ...
  describe("Argument verification", () => {
    describe("child", () => {
      it("throws on no args", () => {
        testShared.assertThrows(testShared.bind(root.child, root),
                                'storage/invalid-argument-count');
      });
      it("throws on null instead of path", () => {
        testShared.assertThrows(testShared.bind(root.child, root, null),
                                'storage/invalid-argument');
      });
      it("throws on number instead of path", () => {
        testShared.assertThrows(testShared.bind(root.child, root, 3),
                                'storage/invalid-argument');
      });
    });
    ...
    describe("toString", () => {
      it("throws on number arg", () => {
        testShared.assertThrows(testShared.bind(root.toString, root, 3),
                                'storage/invalid-argument-count');
      });
    });
    ...
    describe("put", () => {
      ...
      it("throws on no arguments", () => {
        testShared.assertThrows(testShared.bind(child.put, child),
                                'storage/invalid-argument-count');
      });
      ...
      it("throws on number instead of data", () => {
        testShared.assertThrows(testShared.bind(child.put, child, 3),
                                'storage/invalid-argument');
      });
      it("throws null instead of data", () => {
        testShared.assertThrows(testShared.bind(child.put, child, null),
                                'storage/invalid-argument');
      });
      ...
      it("throws when object is supplied instead of string", () => {
        ...
        testShared.assertThrows(
            testShared.bind(child.put, child, blob,
    ...
    describe("putString", () => {
      it("throws on no arguments", () => {
        testShared.assertThrows(testShared.bind(child.putString, child),
                                'storage/invalid-argument-count');
      });
    ...
    describe("delete", () => {
      it("throws on a number arg", () => {
        testShared.assertThrows(testShared.bind(child.delete, child, 3),
                                'storage/invalid-argument-count');
      });
    });
    ...
    describe("getMetadata", () => {
      it("throws on a number arg", () => {
        testShared.assertThrows(testShared.bind(child.getMetadata, child, 3),
                                'storage/invalid-argument-count');
      });
    });
    ...
    describe("updateMetadata", () => {
      it("throws on no args", () => {
        testShared.assertThrows(testShared.bind(child.updateMetadata, child),
                                'storage/invalid-argument-count');
      });
      it("throws on number arg", () => {
        testShared.assertThrows(testShared.bind(child.updateMetadata, child, 3),
                                'storage/invalid-argument');
      });
    ...
    describe("getDownloadURL", () => {
      it("throws on number arg", () => {
        testShared.assertThrows(testShared.bind(child.getDownloadURL, child, 3),
                                'storage/invalid-argument-count');
      });
    });
  });
  ...
  describe("non-root operations", () => {
    ...
    it("putString doesn't throw", () => {
      assert.doesNotThrow(() => {
        ...
        child.putString('data:application/octet-stream;base64,aaaa',
                        StringFormat.DATA_URL);
      });
    });
    it("delete doesn't throw",
       () => { assert.doesNotThrow(() => { child.delete(); }); });
    it("getMetadata doesn't throw",
       () => { assert.doesNotThrow(() => { child.getMetadata(); }); });
    it("updateMetadata doesn't throw", () => {
      assert.doesNotThrow(() => { child.updateMetadata({} as Metadata); });
    });
    it("getDownloadURL doesn't throw",
       () => { assert.doesNotThrow(() => { child.getDownloadURL(); }); });
  });

  describe("root operations", () => {
    it("put throws", () => {
      testShared.assertThrows(root.put.bind(root, new Blob([ 'a' ])),
    ...
    it("delete throws", () => {
      testShared.assertThrows(root.delete.bind(root),
                              'storage/invalid-root-operation');
    });
    it("getMetadata throws", () => {
      testShared.assertThrows(root.getMetadata.bind(root),
                              'storage/invalid-root-operation');
    });
    it("updateMetadata throws", () => {
      testShared.assertThrows(root.updateMetadata.bind(root, {}),
                              'storage/invalid-root-operation');
    });
    it("getDownloadURL throws", () => {
      testShared.assertThrows(root.getDownloadURL.bind(root),
                              'storage/invalid-root-operation');
    });
  });
});
