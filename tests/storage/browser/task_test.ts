...
type RequestHandler = (url: string, method: string,
                       body?: ArrayBufferView|Blob|string, headers?: Headers) =>
    Response;
...
function authWrapperWithHandler(handler: RequestHandler): AuthWrapper {
  ...
  return new AuthWrapper(null, (_1, _2) => { return {} as Reference; },
                         makeRequest, {} as Service, makePool(newSend));
}

function fakeServerHandler(): RequestHandler {
  const stats: {[num: number] :
                    {currentSize : number, finalSize : number}} = {};

  let nextId: number = 0;

  function statusHeaders(status: string,
                         opt_existing?: StringHeaders): StringHeaders {
    if (opt_existing) {
  ...
  function handler(url: string, method: string,
    ...
    if (headers['X-Goog-Upload-Protocol'] === 'resumable') {
      ...
      return {
        ...
        headers : statusHeaders(
            'active', {'X-Goog-Upload-URL' : 'http://example.com?' + thisId})
      };
    }
    ...
    const commands =
        (headers['X-Goog-Upload-Command'] as string).split(',').map(str => {
          return str.trim();
        });
...
describe("Firebase Storage > Upload Task", () => {
  it("Works for a small upload w/ an observer", () => {
    ...
    const task = new UploadTask({} as Reference, authWrapper, testLocation,
                                mappings, smallBlob);
    return fbsPromise.make<void>((resolve, reject) => {
      task.on(TaskEvent.STATE_CHANGED, null,
              error => { assert.fail('Unexpected upload failure'); },
              () => { resolve(null); });
    });
  });
  it("Works for a small upload w/ a promise", () => {
    ...
    const task = new UploadTask({} as Reference, authWrapper, testLocation,
                                mappings, smallBlob);
    return task.then(
        snapshot => { assert.equal(snapshot.totalBytes, smallBlob.size()); });
  });
  it("Works for a small upload canceled w/ a promise", () => {
    ...
    const promise: Promise<string> = task.then<string>(
        ...
        err => { return 'Task failed as expected'; });
    task.cancel();
    return promise;
  });
  it("Works properly with multiple observers", () => {
    ...
    const h3: Unsubscribe =
        (() => {
          let lastState;
          return task.on(TaskEvent.STATE_CHANGED, snapshot => {
    ...
    return fbsPromise.make<void>((resolve, reject) => {
      task.on(TaskEvent.STATE_CHANGED, null,
              error => { assert.fail('Upload failed'); },
              function() {
  ...
  function runNormalUploadTest(blob: FbsBlob): Promise<void> {
    ...
    // Assert functions throw Errors, which means if we're not in the right
    // stack the error might not get reported properly. This function wraps
    // existing assert functions, returning a new function that calls reject
    // with any caught errors. This should make sure errors are reported
    // properly.
      ...
      task.on(TaskEvent.STATE_CHANGED,
              snapshot => {
                ...
                if (lastState !== TaskState.RUNNING &&
                  ...
                  events.push('resume');
                } else if (lastState !== TaskState.PAUSED &&
              ...
              error => { fixedAssertFail('upload failed'); },
              () => {
    ...
    (function() {
      ...
      task.on(TaskEvent.STATE_CHANGED, function(snapshot) {
        ...
        if (lastState !== TaskState.PAUSED && state === TaskState.PAUSED) {
          events.push('timeout');
          setTimeout(function() { task.resume(); }, 200);
        }
        lastState = state;
      });
    })();
    ...
    task.on(TaskEvent.STATE_CHANGED, null, null, () => {
      ...
      const task2 = new UploadTask({} as Reference, authWrapper, testLocation,
                                   mappings, blob);
      const events2 = [];

      (function() {
        let lastState;
        task2.on(TaskEvent.STATE_CHANGED,
                 snapshot => {
                   const state = snapshot.state;
                   if (lastState !== TaskState.RUNNING &&
                       state === TaskState.RUNNING) {
  ...
  describe("Argument verification", () => {
    ...
    describe("on", () => {
      ...
      it("Throws on 5 args", () => {
        assertThrows(fbsBind(task.on, task, TaskEvent.STATE_CHANGED, null, null,
                             null, 1),
      ...
      it("Throws on an empty object arg", () => {
        assertThrows(fbsBind(task.on, task, TaskEvent.STATE_CHANGED, {}),
                     'storage/invalid-argument');
      });
    });
    describe("subscribe returned from on", () => {
      it("Throws on no args", () => {
        assertThrows(fbsBind(task.on(TaskEvent.STATE_CHANGED), null),
                     'storage/invalid-argument-count');
      });
      it("Throws on 4 args", () => {
        assertThrows(fbsBind(task.on(TaskEvent.STATE_CHANGED), null, null, null,
                             null, 1),
                     'storage/invalid-argument-count');
      ...
      it("Throws on a single null arg", () => {
        assertThrows(fbsBind(task.on(TaskEvent.STATE_CHANGED), null, null),
                     'storage/invalid-argument');
      });
    });
    describe("resume", () => {
      it("Throws on a number", () => {
        assertThrows(fbsBind(task.resume, task, 3),
                     'storage/invalid-argument-count');
      });
    });
    describe("pause", () => {
      it("Throws on a number", () => {
        assertThrows(fbsBind(task.pause, task, 3),
                     'storage/invalid-argument-count');
      });
    });
    describe("cancel", () => {
      it("Throws on a number", () => {
        assertThrows(fbsBind(task.cancel, task, 3),
                     'storage/invalid-argument-count');
      });
    });
  });
