...
import {
  dataFromString,
  StringFormat
} from '../../../src/storage/implementation/string';
...
describe("Firebase Storage > String", () => {
  it("Encodes raw strings with ascii correctly", () => {
    ...
    assertUint8ArrayEquals(new Uint8Array([
  ...
  it("Encodes raw strings with 2-byte UTF8 codepoints correctly", () => {
    const str = 'aa\u0089';
    assertUint8ArrayEquals(new Uint8Array([ 0x61, 0x61, 0xC2, 0x89 ]),
                           dataFromString(StringFormat.RAW, str).data);
  });
  it("Encodes raw strings with 3-byte UTF8 codepoints correctly", () => {
    const str = 'aa\uff7c';
    assertUint8ArrayEquals(new Uint8Array([ 0x61, 0x61, 0xEF, 0xBD, 0xBC ]),
                           dataFromString(StringFormat.RAW, str).data);
  });
  ...
  it("Encodes base64 strings correctly", () => {
    ...
    assertUint8ArrayEquals(base64Bytes,
  ...
  it("Rejects invalid base64 strings", () => {
    ...
    assertThrows(function() { dataFromString(StringFormat.BASE64, str); },
  ...
  it("Rejects invalid base64url strings", () => {
    const str = 'CpYlM1+XsGxTd1n6izHMU/yY3Bw=';
    assertThrows(function() { dataFromString(StringFormat.BASE64URL, str); },
                 'storage/invalid-format');
  });
  ...
  it("Encodes non-base64 data URLs with no content type correctly", () => {
    const str = 'data:,aaaa';
    const data = dataFromString(StringFormat.DATA_URL, str);
    assertUint8ArrayEquals(new Uint8Array([ 0x61, 0x61, 0x61, 0x61 ]),
                           data.data);
  ...
  it("Encodes non-base64 data URLs with content type correctly", () => {
    ...
    const data = dataFromString(StringFormat.DATA_URL, str);
    assertUint8ArrayEquals(new Uint8Array([ 0x61, 0x72, 0x73, 0x74 ]),
                           data.data);
  ...
  it("Rejects data URLs with invalid URL encodings", () => {
    const str = 'data:,%%0';
    assertThrows(function() { dataFromString(StringFormat.DATA_URL, str); },
                 'storage/invalid-format');
  });
