...
describe('Firebase Messaging > tokenManager.getSavedToken()', function() {
  ...
  it('should handle bad send ID input', function() {
    ...
    const promises = badInputs.map(badInput => {
      ...
      return globalTokenManager.getSavedToken(badInput, new FakeRegistration())
          .then(
              () => {
                throw new Error(
                    'Expected getSavedToken to reject the promise.');
              },
              err => {
  ...
  it('should handle bad registration input', function() {
    ...
    const promises = badInputs.map(badInput => {
      globalTokenManager = new TokenManager();
      return globalTokenManager.getSavedToken('1234567890', badInput)
          .then(
              () => {
                throw new Error(
                    'Expected getSavedToken to reject the promise.');
  ...
  it('should handle sw mismatch', function() {
    ...
    Object.defineProperty(registration, 'scope', {value : swScope});

    globalTokenManager = new TokenManager();
    return globalTokenManager.getSavedToken('1234567890', registration)
        .then(token => { assert.equal(undefined, token); });
  });
  ...
  it('should handle sender ID mismatch', function() {
    ...
    const registration = new FakeRegistration();
    Object.defineProperty(registration, 'scope', {value : details.swScope});
    ...
    globalTokenManager = new TokenManager();
    return globalTokenManager.getSavedToken(originalSenderID, registration)
        .then(token => { assert.equal(details.fcmToken, token); })
        .then(() => {
          return globalTokenManager.getSavedToken(sencondSenderID,
                                                  registration);
        })
        .then(token => { assert.equal(undefined, token); });
  });
  ...
  it('should handle subscription', function() {
    ...
    Object.defineProperty(
        ...
        {value : {getSubscription : () => { return Promise.resolve(null); }}});
    ...
    return globalTokenManager.getSavedToken(senderId, registration)
            ...
            value : {
              getSubscription : () => {
                return Promise.reject(
                    new Error('Unknown service worker error'));
              }
            }
          });
        ...
        .then(token => { assert.equal('current-token', token); });
  });
});
