...
import VapidDetailsModel from
    '../../../src/messaging/models/vapid-details-model';

import dbHelpers from './db-helper';

describe(
    'Firebase Messaging > VapidDetailsModel.getVapidFromSWScope()', function() {
      ...
      it('should get vapid key', function() {
        ...
        return vapidModel.getVapidFromSWScope(EXAMPLE_SCOPE)
            .then((vapidKey) => {
              ...
              return vapidModel.saveVapidDetails(EXAMPLE_SCOPE,
                                                 EXAMPLE_VAPID_STRING);
            })
            .then(
                () => { return vapidModel.getVapidFromSWScope(EXAMPLE_SCOPE); })
            .then((vapidKey) => {
