...
import TokenDetailsModel from
    '../../../src/messaging/models/token-details-model';
...
describe('Firebase Messaging > TokenDetailsModel.deleteToken()', function() {
  ...
  it('should delete current token', function() {
    ...
    return tokenModel.saveTokenDetails(EXAMPLE_INPUT)
        .then(() => { return tokenModel.deleteToken(EXAMPLE_INPUT.fcmToken); })
        .then(details => {
          const subscriptionKeys = [ 'endpoint', 'auth', 'p256dh' ];
          const subscriptionValues = {
            endpoint : EXAMPLE_INPUT.subscription.endpoint,
            auth :
                arrayBufferToBase64(EXAMPLE_INPUT.subscription.getKey('auth')),
            p256dh :
                arrayBufferToBase64(EXAMPLE_INPUT.subscription.getKey('p256dh'))
          };
        ...
        .then(tokenDetails => { assert.equal(null, tokenDetails); });
  });
