...
import arrayBufferToBase64 from
    '../../../src/messaging/helpers/array-buffer-to-base64';
...
describe('Firebase Messaging > tokenManager.createToken()', function() {
  ...
  afterEach(function() {
    ...
    return Promise.all(
        [ globalTokenManager.closeDatabase(), dbTMHelper.closeDatabase() ]);
  });
  ...
  it('should test bad input', function() {
    ...
    const badInputs = [
      [ undefined, undefined ], [ EXAMPLE_SENDER_ID, undefined ],
      [ undefined, exampleSWReg ], [ null, null ], [ EXAMPLE_SENDER_ID, null ],
      [ null, exampleSWReg ], [ {}, exampleSWReg ], [ [], exampleSWReg ],
      [ true, exampleSWReg ], [ 1, exampleSWReg ], [ '', exampleSWReg ],
      [ EXAMPLE_SENDER_ID, {} ], [ EXAMPLE_SENDER_ID, [] ],
      [ EXAMPLE_SENDER_ID, true ], [ EXAMPLE_SENDER_ID, 1 ],
      [ EXAMPLE_SENDER_ID, '' ]
    ];
    ...
    return badInputs.reduce((promiseChain, badInput) => {
      return promiseChain.then(() => {
        return globalTokenManager.createToken(badInput[0], badInput[1])
            .then(() => { throw new Error('Bad input should have thrown'); },
                  err => {
                    switch (err.code) {
  ...
  it('should handle failing getSubscription', function() {
    const activatedRegistration =
        makeFakeSWReg('active', {state : 'activated'});

    Object.defineProperty(activatedRegistration, 'pushManager', {
      value : {
        getSubscription : () => { return Promise.reject('Unknown error.'); }
      }
    });
    ...
    return globalTokenManager
        ...
        .then(() => { throw new Error('This should have rejected'); },
              err => { expect(err).to.equal('Unknown error.'); });
  });
  ...
  it('should handle failing subscribe call', function() {
    ...
    Object.defineProperty(activatedRegistration, 'pushManager', {
      value : {
        ...
        subscribe :
            (options) => { return Promise.reject(new Error('Unknown Error')); }
      }
    });

    const subscribeStub =
        sinon.stub(TokenManager.prototype, 'subscribeToFCM')
    ...
    return globalTokenManager
        .createToken(EXAMPLE_SENDER_ID, activatedRegistration)
        .then(() => { throw new Error('Expected createToken to throw.'); },
              err => { expect(err.message).to.equal('Unknown Error'); });
  });
  ...
  it('should use an existing subscription', function() {
    ...
    return globalTokenManager
        ...
        .then(allDetails => {
          ...
          expect(details.fcmPushSet)
              .to.equal(EXAMPLE_FCM_TOKEN_DETAILS.pushSet);
        ...
        .then(() => {
          return globalTokenManager.getSavedToken(EXAMPLE_SENDER_ID,
  ...
  it('should handle valid flow', function() {
    ...
    stubs.push(sinon.stub(TokenManager.prototype, 'subscribeToFCM')
                   .onCall(0)
    ...
    return validCombos.reduce((promiseChain, validCombo) => {
      return promiseChain.then(() => {
        return globalTokenManager
            ...
            .then(
                token => { expect(token).to.equal(validCombo.expectedToken); })
            .then(() => {
              // Ensure details are saved correctly
            ...
            .then(details => {
              ...
              expect(arrayBufferToBase64(P256DH_BUFFER))
                  .to.equal(details.p256dh);
            })
            .then(() => {
              return globalTokenManager.getSavedToken(validCombo.senderId,
                                                      validCombo.swReg);
            })
            .then(
                token => { expect(validCombo.expectedToken).to.equal(token); });
      });
    }, Promise.resolve());
  });
  ...
  it('should handle sender ID difference', function() {
    ...
    Object.defineProperty(activatedRegistration, 'pushManager', {
      value : {
        getSubscription : () => { return Promise.resolve(null); },
        subscribe :
            (options) => { return Promise.resolve(EXAMPLE_SUBSCRIPTION); }
      }
    });
    ...
    methodStub.withArgs(EXAMPLE_SENDER_ID, EXAMPLE_SUBSCRIPTION)
        .returns(Promise.resolve(EXAMPLE_FCM_TOKEN_DETAILS));
    methodStub.withArgs(secondSenderId, EXAMPLE_SUBSCRIPTION)
    ...
    return globalTokenManager
        ...
        .then(() => {
          return globalTokenManager.createToken(secondSenderId,
                                                activatedRegistration);
        })
        .then(token => { expect(secondDetails.token).to.equal(token); });
  });
});
