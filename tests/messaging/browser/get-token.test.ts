...
import ControllerInterface from
    '../../../src/messaging/controllers/controller-interface';
import SWController from '../../../src/messaging/controllers/sw-controller';
import WindowController from
    '../../../src/messaging/controllers/window-controller';
import DefaultSW from '../../../src/messaging/models/default-sw';
import Errors from '../../../src/messaging/models/errors';
import NotificationPermission from
    '../../../src/messaging/models/notification-permission';
...
describe('Firebase Messaging > *Controller.getToken()', function() {
  ...
  const app = makeFakeApp({messagingSenderId : EXAMPLE_SENDER_ID});
  ...
  const mockGetReg = fakeReg => {
    servicesToTest.forEach(serviceClass => {
      const getSwMock =
          sinon.stub(serviceClass.prototype, 'getSWRegistration_');
  ...
  it('should throw on unsupported browsers', function() {
    const isSupportedStub =
        sinon.stub(WindowController.prototype, 'isSupported_');
    ...
    return messagingService.getToken().then(
        ...
        err => {
          assert.equal('messaging/' + Errors.codes.UNSUPPORTED_BROWSER,
                       err.code);
        });
  });
  ...
  it('should handle a failure to get registration', function() {
    const notificationStub =
        sinon.stub(ControllerInterface.prototype, 'getNotificationPermission_');
    ...
    return messagingService.getToken()
        .then(() => { throw new Error('Expected getToken to throw '); },
              err => {
                assert.equal('messaging/' +
                                 Errors.codes.FAILED_DEFAULT_REGISTRATION,
                             err.code);
              })
        .then(() => { messagingService.delete(); });
  });
  ...
  it('should handle the notification permission', function() {
    ...
    return servicesToTest.reduce((chain, ServiceClass) => {
      ...
      return chain.then(() => { return serviceInstance.getToken(); })
          .then(() => { throw new Error('Expected getToken to throw '); },
                err => {
                  assert.equal('messaging/' +
                                   Errors.codes.NOTIFICATIONS_BLOCKED,
                               err.code);
                })
          .then(() => { return serviceInstance.getToken(); })
          .then(token => { assert.equal(null, token); });
    }, Promise.resolve());
  });
  ...
  it('should get saved token', function() {
    ...
    return Promise.all(servicesToTest.map(ServiceClass => {
      const serviceInstance = new ServiceClass(app);
      return serviceInstance.getToken().then(
          token => { assert.equal(EXAMPLE_FCM_TOKEN, token); });
    }));
  });
