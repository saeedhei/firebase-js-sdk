...
describe("Firebase App Class", () => {
  ...
  beforeEach(() => { firebase = createFirebaseNamespace(); });

  it("No initial apps.", () => { assert.equal(firebase.apps.length, 0); });
  ...
  it("Can delete App.", () => {
    let app = firebase.initializeApp({});
    assert.equal(firebase.apps.length, 1);
    return app.delete().then(() => { assert.equal(firebase.apps.length, 0); });
  });
  ...
  it("Register App Hook", (done) => {
    ...
    firebase.INTERNAL.registerService(
        'test', (app: FirebaseApp) => { return new TestService(app); },
        undefined,
  ...
  it("Duplicate DEFAULT initialize is an error.", () => {
    firebase.initializeApp({});
    assert.throws(() => { firebase.initializeApp({}); },
                  /\[DEFAULT\].*exists/i);
  });

  it("Duplicate named App initialize is an error.", () => {
    firebase.initializeApp({}, 'abc');
    assert.throws(() => { firebase.initializeApp({}, 'abc'); },
                  /'abc'.*exists/i);
  });
  ...
  it("Error to use app after it is deleted.", () => {
    ...
    return app.delete().then(() => {
      assert.throws(() => { console.log(app.name); }, /already.*deleted/);
    });
  });
  ...
  it("Error to get uninitialized app using Object.prototype member name.",
     () => {
       assert.throws(() => { firebase.app('toString'); },
                     /'toString'.*created/i);
     });
  ...
  it("Can lazy load a service", () => {
    ...
    // Service should only be created once... regardless of how you invoke the
    // function
  ...
  it("Can lazy register App Hook", (done) => {
    ...
    firebase.INTERNAL.registerService(
        'lazyServiceWithHook',
        (app: FirebaseApp) => { return new TestService(app); }, undefined,
        (event: string, app: FirebaseApp) => {
          assert.equal(event, events[hookEvents]);
          hookEvents += 1;
          if (hookEvents === events.length) {
  ...
  it('Can register multiple instances of some services', () => {
    ...
    firebase.INTERNAL.registerService('multiInstance', (...args) => {
    ...
    assert.strictEqual(
        service2, (firebase.app() as any).multiInstance(serviceIdentifier));

    // Ensure that the two services **are not equal**
    assert.notStrictEqual(service.instanceIdentifier,
  ...
  it(`Should return the same instance of a service if a service doesn't support multi instance`,
     () => {
       ...
       const service2 =
           (firebase.app() as any).singleInstance(serviceIdentifier);

       // Ensure that the two services **are equal**
       assert.strictEqual(service.instanceIdentifier,
                          service2.instanceIdentifier,
                          '`instanceIdentifier` is not being set correctly');
  ...
  it(`Should pass null to the factory method if using default instance`, () => {
    ...
    firebase.INTERNAL.registerService('testService', (...args) => {
      const [app, , instanceIdentifier] = args;
      assert.isUndefined(instanceIdentifier,
  ...
  it(`Should extend INTERNAL per app instance`, () => {
    ...
    firebase.INTERNAL.registerService(
        'test', (app: FirebaseApp, extendApp: any) => {
    ...
    return app.INTERNAL.getToken()
        ...
        .then(token => { assert.equal('tokenFor1', token.accessToken); });
  });
  ...
  describe("Check for bad app names", () => {
    ...
    for (let data of tests) {
      it("where name == '" + data + "'", () => {
        assert.throws(() => { firebase.initializeApp({}, data as string); },
                      /Illegal app name/i);
        ;
      });
    }
...
class TestService implements FirebaseService {
  ...
  delete(): Promise<void> {
    return new Promise(
        (resolve: (v?: void) => void) => { setTimeout(() => resolve(), 10); });
  }
}
