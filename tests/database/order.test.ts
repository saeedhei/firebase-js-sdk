...
describe('Order Tests', function() {
  // Kind of a hack, but a lot of these tests are written such that they'll fail
  // if run before we're connected to Firebase because they do a bunch of sets
  // and then a listen and assume that they'll arrive in that order.  But if we
  // aren't connected yet, the "reconnection" code will send them in the
  // opposite order.
  beforeEach(function() {
  ...
  it("Push a bunch of data, enumerate it back; ensure order is correct.",
     async function() {
  ...
  it("Push a bunch of paths, then write; ensure order is correct.",
     async function() {
       const node = (getRandomNode() as Reference);
       const paths = [];
  ...
  it("Push a bunch of data, reconnect, read it back; ensure order is chronological.",
     async function() {
  ...
  it("Push a bunch of data with explicit priority, reconnect, read it back; ensure order is correct.",
     async function() {
       const nodePair = (getRandomNode(2) as Reference[]);
       let expected;
       ...
       // local SETs are visible immediately, but the second node is in a
       // separate repo, so it is considered remote. We need confirmation that
       // the server has gotten all the data before we can expect to receive it
       // all
  ...
  it("Push data with exponential priority and ensure order is correct.",
     async function() {
       const nodePair = (getRandomNode(2) as Reference[]);
       let expected;
       ...
       for (let i = 0; i < 10; i++) {
         ...
         pushedNode.setWithPriority(
             i, 111111111111111111111111111111 / Math.pow(10, i),
             function() { ++nodesSet });
       }
  ...
  it.skip("Receive child_moved event when priority changes.", async function() {
    ...
    // eventHelper.watchesInitializedWaiter);

    const eventHelper = eventTestHelper([
      [ node, [ 'child_added', 'a' ] ], [ node, [ 'value', '' ] ],
      [ node, [ 'child_added', 'b' ] ], [ node, [ 'value', '' ] ],
      [ node, [ 'child_added', 'c' ] ], [ node, [ 'value', '' ] ],
      [ node, [ 'child_moved', 'a' ] ], [ node, [ 'child_changed', 'a' ] ],
      [ node, [ 'value', '' ] ]
    ]);
  ...
  it.skip("Can reset priority to null.", async function() {
    ...
    eventHelper = eventTestHelper([
      [ node, [ 'child_added', 'a' ] ], [ node, [ 'child_added', 'b' ] ],
      [ node, [ 'value', '' ] ]
    ]);
    ...
    eventHelper.addExpectedEvents([
      [ node, [ 'child_moved', 'b' ] ], [ node, [ 'child_changed', 'b' ] ],
      [ node, [ 'value', '' ] ]
    ]);
  ...
  it("Verify order of mixed numbers / strings / no priorities.", async function() {
    ...
    const nodeAndPriorities = [
      "alpha42",     "zed",    "noPriorityC", null,    "num41",       500,
      "noPriorityB", null,     "num80",       4000.1,  "num50",       4000,
      "num10",       24,       "alpha41",     "zed",   "alpha20",     "horse",
      "num20",       123,      "num70",       4000.01, "noPriorityA", null,
    ...
    let output = "";
    snap.forEach(function(n) { output += n.key + ", "; });
  ...
  it("Case 595: Should not get child_moved event when deleting prioritized grandchild.",
     function() {
       ...
       let moves = 0;
       f.on('child_moved', function() { moves++; });
