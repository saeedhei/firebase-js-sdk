...
describe('Crawler Support', function() {
  ...
  afterEach(function() { tokenProvider.setToken(null); });
  ...
  it('set() is a no-op', function(done) {
    normalRef.child('leaf').on('value',
                               function(s) { expect(s.val()).to.equal(42); });

    restRef.child('leaf').set('hello');

    // We need to wait long enough to be sure that our 'hello' didn't actually
    // get set, but there's no good way to do that.  So we just do a couple
    // round-trips via the REST client and assume that's good enough.
  ...
  it('set() is a no-op (Promise)', function() {
    // This test mostly exists to make sure restRef really is using
    // ReadonlyRestClient and we're not accidentally testing a normal Firebase
    // connection.
