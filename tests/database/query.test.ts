...
import {getPath, getRandomNode, pause} from "./helpers/util";
...
describe('Query Tests', function() {
  ...
  EventReceiver.prototype.onValue = function() { this.gotValue = true; };
  ...
  it('Passing invalidKeys to startAt / endAt throws.', function() {
    ...
    const badKeys = [
      '.test', 'test.', 'fo$o', '[what', 'ever]', 'ha#sh', '/thing', 'th/ing',
      'thing/'
    ];
    // Changed from basic array iteration to avoid closure issues accessing
    // mutable state
  ...
  it('Query.queryIdentifier works.', function() {
    const path = (getRandomNode() as Reference);
    const queryId = function(query) { return query.queryIdentifier(query); };
  ...
  it('Query.isEqual works.', function() {
    ...
    expect(rootRef.child('child').isEqual(childRef), 'Query.isEqual - 4')
        .to.be.true;
    ...
    // expect(rootRef.isEqual(rootRefDifferentRepo), 'Query.isEqual -
    // 5').to.be.false;
    // expect(childRef.isEqual(rootRefDifferentRepo.child('child')),
    // 'Query.isEqual - 6').to.be.false;
    ...
    const childQueryStartAt1EndAt2 =
        childQueryOrderedByTimestamp.startAt(1).endAt(2);

    // Equivalent queries
    expect(childRef.isEqual(childQueryLast25.ref), 'Query.isEqual - 9')
        .to.be.true;
    expect(childQueryLast25.isEqual(childRef.limitToLast(25)),
           'Query.isEqual - 10')
        .to.be.true;
    expect(childQueryStartAt1EndAt2.isEqual(
               childQueryOrderedByTimestamp.startAt(1).endAt(2)),
           'Query.isEqual - 11')
        .to.be.true;

    // Non-equivalent queries
    expect(childQueryLast25.isEqual(childRef), 'Query.isEqual - 12')
        .to.be.false;
    expect(childQueryLast25.isEqual(childQueryOrderedByKey),
           'Query.isEqual - 13')
        .to.be.false;
    expect(childQueryLast25.isEqual(childQueryOrderedByPriority),
           'Query.isEqual - 14')
        .to.be.false;
    expect(childQueryLast25.isEqual(childQueryOrderedByTimestamp),
           'Query.isEqual - 15')
        .to.be.false;
    expect(childQueryOrderedByKey.isEqual(childQueryOrderedByPriority),
           'Query.isEqual - 16')
        .to.be.false;
    expect(childQueryOrderedByKey.isEqual(childQueryOrderedByTimestamp),
           'Query.isEqual - 17')
        .to.be.false;
    expect(childQueryStartAt1.isEqual(childQueryStartAt2), 'Query.isEqual - 18')
        .to.be.false;
    expect(childQueryStartAt1.isEqual(childQueryStartAt1EndAt2),
           'Query.isEqual - 19')
        .to.be.false;
    expect(childQueryEndAt2.isEqual(childQueryStartAt2), 'Query.isEqual - 20')
        .to.be.false;
    expect(childQueryEndAt2.isEqual(childQueryStartAt1EndAt2),
           'Query.isEqual - 21')
        .to.be.false;
  });
  ...
  it('Query.off can be called on the default query.', function() {
    ...
    const callback = function() { eventFired = true; };
  ...
  it('Query.off can be called without a callback specified.', function() {
    ...
    const callback1 = function() { eventFired = true; };
    const callback2 = function() { eventFired = true; };
  ...
  it('Query.off respects provided context (for value events).', function() {
    ...
    const a = new EventReceiver(), b = new EventReceiver();
  ...
  it('Set a limit of 5, add a bunch of nodes, ensure only last 5 items are sent from server.',
     async function() {
       const node = (getRandomNode() as Reference);
       await node.set({});
       ...
       node.limitToLast(5).on('value', snap => { ea.addEvent(snap); });
  ...
  it('Set various limits, ensure resulting data is correct.', async function() {
    ...
    return Promise.all(tasks.map(async task => {
      ...
      const ea = EventAccumulatorFactory.waitsForCount(1);
      query.on('value', snap => { ea.addEvent(snap.val()); });
      const [newVal] = await ea.promise;
      expect(newVal).to.deep.equal(val);
    }));
  });

  it('Set various limits with a startAt name, ensure resulting data is correct.',
     async function() {
       const node = (getRandomNode() as Reference);
  ...
  it('Set various limits with a endAt name, ensure resulting data is correct.',
     async function() {
       const node = (getRandomNode() as Reference);
  ...
  it('Set various limits with a startAt name, ensure resulting data is correct from the server.',
     async function() {
       const node = (getRandomNode() as Reference);
       ...
       const tasks: TaskList = [
         ...
         [ node.startAt(null, 'c').limitToFirst(1), {c : 3} ],
         [ node.startAt(null, 'b').limitToFirst(1), {b : 2} ],
         // NOTE: technically there is a race condition here. The
         // limitToFirst(1) query will return a single value, which will be
         // raised for the limitToFirst(2) callback as well, if it exists
         // already. However, once the server gets the limitToFirst(2) query, it
         // will send more data and the correct state will be returned.
         [ node.startAt(null, 'b').limitToFirst(2), {b : 2, c : 3} ],
         [ node.startAt(null, 'b').limitToFirst(3), {b : 2, c : 3} ],
       ];
  ...
  it('Set limit, ensure child_removed and child_added events are fired when limit is hit.',
     function() {
       const node = (getRandomNode() as Reference);
       let added = '', removed = '';
       node.limitToLast(2).on('child_added',
                              function(snap) { added += snap.key + ' ' });
       node.limitToLast(2).on('child_removed',
                              function(snap) { removed += snap.key + ' ' });
  ...
  it('Set limit, ensure child_removed and child_added events are fired when limit is hit, using server data',
     async function() {
       const node = (getRandomNode() as Reference);
  ...
  it('Set start and limit, ensure child_removed and child_added events are fired when limit is hit.',
     function() {
       ...
       let added = '', removed = '';
       node.startAt(null, 'a').limitToFirst(2).on(
           'child_added', function(snap) { added += snap.key + ' ' });
       node.startAt(null, 'a').limitToFirst(2).on(
           'child_removed', function(snap) { removed += snap.key + ' ' });
       node.set({a : 1, b : 2, c : 3});
  ...
  it('Set start and limit, ensure child_removed and child_added events are fired when limit is hit, using server data',
     async function() {
       ...
       let added = '', removed = '';
       node.startAt(null, 'a').limitToFirst(2).on('child_added',
                                                  function(snap) {
                                                    added += snap.key + ' ';
                                                    ea.addEvent();
                                                  });
  ...
  it("Set start and limit, ensure child_added events are fired when limit isn't hit yet, using server data",
     async function() {
       const node = (getRandomNode() as Reference);
  ...
  it('Set a limit, ensure child_removed and child_added events are fired when limit is satisfied and you remove an item.',
     async function() {
       const node = (getRandomNode() as Reference);
       const ea = EventAccumulatorFactory.waitsForCount(1);
  ...
  it('Set a limit, ensure child_removed and child_added events are fired when limit is satisfied and you remove an item. Using server data',
     async function() {
       const node = (getRandomNode() as Reference);
  ...
  it('Set a limit, ensure child_removed events are fired when limit is satisfied, you remove an item, and there are no more. Using server data',
     async function() {
       const node = (getRandomNode() as Reference);
       const ea = EventAccumulatorFactory.waitsForCount(2);
       let added = '';
  ...
  it('Ensure startAt / endAt with priority work with server data.',
     async function() {
       const node = (getRandomNode() as Reference);
  ...
  it('Ensure startAt / endAt with priority and name work with server data',
     async function() {
       const node = (getRandomNode() as Reference);
  ...
  it('Ensure startAt / endAt with priority and name works (2). With server data',
     async function() {
       const node = (getRandomNode() as Reference);
  ...
  it('Set a limit, add some nodes, ensure prevName works correctly. With server data',
     async function() {
       const node = (getRandomNode() as Reference);
  ...
  it('Set a limit, move some nodes, ensure prevName works correctly, with server data',
     async function() {
       const node = (getRandomNode() as Reference);
       let moved = '';
  ...
  it('Numeric priorities: Set a limit, move some nodes, ensure prevName works correctly. With server data',
     async function() {
       const node = (getRandomNode() as Reference);
       let moved = '';
  ...
  it('Set a limit, add a bunch of nodes, ensure remote events are correct.',
     async function() {
  ...
  it("Limit on unsynced node fires 'value'.", function(done) {
    const f = (getRandomNode() as Reference);
    f.limitToLast(1).on('value', function() { done(); });
  });
  ...
  it('Filtering to only null priorities works.', async function() {
    ...
    f.startAt(null).endAt(null).on('value',
                                   snap => { snapAcc.addEvent(snap.val()); });
  ...
  it('null priorities included in endAt(2).', async function() {
    ...
    const ea = EventAccumulatorFactory.waitsForCount(1);
    f.endAt(2).on('value', snap => { ea.addEvent(snap.val()); });
  ...
  it('null priorities not included in startAt(2).', async function() {
    ...
    f.startAt(2).on('value', snap => { ea.addEvent(snap.val()); });
  ...
  function dumpListens(node: Query) {
    ...
    for (let i = 0; i < listenPaths.length; i++) {
      ...
      if (queryIds.length > 0) {
        dumpPieces.push(listenPaths[i].substring(nodePath.length) + ':' +
                        queryIds.join(','));
      }
    }
  ...
  it('Dedupe queried listens: multiple queried listens; no dupes', function() {
    ...
    expect(dumpListens(node))
        .to.equal(':{"l":1,"vf":"r"};/a:{"l":1,"vf":"r"},{"l":5,"vf":"r"}');
  ...
  it('Dedupe queried listens: listen on parent of queried children.',
     function() {
       ...
       const aLim1On =
           node.child('a').limitToLast(1).on('value', function() {});
       expect(dumpListens(node)).to.equal('/a:{"l":1,"vf":"r"}');

       const bLim1On =
           node.child('b').limitToLast(1).on('value', function() {});
       expect(dumpListens(node))
           .to.equal('/a:{"l":1,"vf":"r"};/b:{"l":1,"vf":"r"}');
  ...
  it('Limit with mix of null and non-null priorities.', function() {
    ...
    const children = [];
    node.limitToLast(5).on(
        'child_added', function(childSnap) { children.push(childSnap.key); });
  ...
  it('Limit with mix of null and non-null priorities using server data',
     async function() {
       const node = <Reference>getRandomNode();
  ...
  it('.on() with a context works.', function() {
    ...
    const ListenerDoohickey = function() { this.snap = null; };
    ListenerDoohickey.prototype.onEvent = function(snap) { this.snap = snap; };
  ...
  it('handles an update that deletes the entire window in a query', function() {
    ...
    const snaps = [];
    ref.limitToLast(2).on('value', function(snap) { snaps.push(snap.val()); });
  ...
  it('handles an out-of-view query on a child', function() {
    ...
    let parent = null;
    ref.limitToLast(1).on('value', function(snap) { parent = snap.val(); });

    let child = null;
    ref.child('a').on('value', function(snap) { child = snap.val(); });
  ...
  it('handles diverging views', function() {
    ...
    let c = null;
    ref.limitToLast(1).endAt(null, 'c').on('value',
                                           function(snap) { c = snap.val(); });

    let d = null;
    ref.limitToLast(1).endAt(null, 'd').on('value',
                                           function(snap) { d = snap.val(); });
  ...
  it('.startAt().limitToFirst(1) and then remove first child (case 1664).',
     async function() {
       const ref = (getRandomNode() as Reference);
       ref.set({a : 1, b : 2});
  ...
  it('handles multiple queries on the same node', async function() {
    ...
    await ref.set({a : 1, b : 2, c : 3, d : 4, e : 5, f : 6});
  ...
  it('handles once called on a node with a default listener', async function() {
    ...
    ref.on('value', function(snap) { ea.addEvent(); });
  ...
  it('handles once called on a node with a default listener and non-complete limit',
     async function() {
  ...
  it('Remote remove triggers events.', function(done) {
    const refPair = getRandomNode(2), writeRef = refPair[0],
          readRef = refPair[1];

    writeRef.set({a : 'a', b : 'b', c : 'c', d : 'd', e : 'e'}, function() {

      // Wait to get the initial data, and then remove 'c' remotely and wait for
      // new data.
      let count = 0;
  ...
  it('complex update() at query root raises correct value event', function(
    ...
    let readerLoaded = false, numEventsReceived = 0;
    writer.child('foo').set(
        {a : 1, b : 2, c : 3, d : 4, e : 5}, function(error, dummy) {
          reader.child('foo').startAt().limitToFirst(4).on(
              'value', function(snapshot) {
                ...
                if (!readerLoaded) {
                  ...
                  // We expect to get null values for b and d, along with the
                  // new children and updated value for c
                  writer.child('foo').update(
                      {b : null, c : 'a', cc : 'new', cd : 'new2', d : 'gone'});
                } else {
                  done();
                  expect(val).to.deep.equal(
                      {a : 1, c : 'a', cc : 'new', cd : 'new2'});
                }
              });
        });
  });
  ...
  it('update() at query root raises correct value event', function(done) {
    ...
    let readerLoaded = false, numEventsReceived = 0;
    writer.child('foo').set(
        {'bar' : 'a', 'baz' : 'b', 'bam' : 'c'}, function(error, dummy) {
  ...
  it('listen for child_remove events when parent removed', function(done) {
    ...
    let numEventsReceived = 0, gotA = false, gotB = false, gotC = false,
        readerLoaded = false;
    writer.set({a : 1, b : 'b', c : {'deep' : 'path', 'of' : {'stuff' : true}}},
               function(error, dummy) {
  ...
  it('Queries behave wrong after .once().', async function() {
    ...
    readRef.on('child_removed', function() { expect(false).to.be.true; });
  ...
  it('Case 2003: Correctly get events for startAt/endAt queries when priority changes.',
     function() {
       const ref = (getRandomNode() as Reference);
       const addedFirst = [], removedFirst = [], addedSecond = [],
             removedSecond = [];
       ref.startAt(0).endAt(10).on(
           'child_added', function(snap) { addedFirst.push(snap.key); });
       ref.startAt(0).endAt(10).on(
           'child_removed', function(snap) { removedFirst.push(snap.key); });
       ref.startAt(10).endAt(20).on(
           'child_added', function(snap) { addedSecond.push(snap.key); });
       ref.startAt(10).endAt(20).on(
           'child_removed', function(snap) { removedSecond.push(snap.key); });
  ...
  it('Behaves with diverging queries', async function() {
    ...
    await writer.set({a : {b : 1, c : 2}, e : 3});
  ...
  it('Priority-only updates are processed correctly by server.',
     async function() {
       const refPair = (getRandomNode(2) as Reference[]), readRef = refPair[0],
             writeRef = refPair[1];
  ...
  it('Server: Test re-listen', function(done) {
    const refPair = (getRandomNode(2) as Reference[]), ref = refPair[0],
          ref2 = refPair[1];
    ref.set({a : 'a', b : 'b', c : 'c', d : 'd', e : 'e', f : 'f', g : 'g'});

    let before;
    ref.startAt(null, 'a').endAt(null, 'b').on(
        'value', function(b) { before = b.val(); });
  ...
  it('Server: Test re-listen 2', function(done) {
    const refPair = getRandomNode(2), ref = refPair[0], ref2 = refPair[1];
    ref.set({a : 'a', b : 'b', c : 'c', d : 'd', e : 'e', f : 'f', g : 'g'});

    let before;
    ref.startAt(null, 'b').limitToFirst(3).on(
        'value', function(b) { before = b.val(); });

    ref.child('aa').update(
        {'a' : 5, 'aa' : 4, 'b' : 7, 'c' : 4, 'd' : 4, 'dd' : 3}, function() {
  ...
  it('Server: Test re-listen 3', function(done) {
    ...
    let before;
    ref.limitToLast(3).on('value', function(b) { before = b.val(); });
  ...
  it('Server limit below limit works properly.', async function() {
    const refPair = <Reference[]>getRandomNode(2), readRef = refPair[0],
          writeRef = refPair[1];
    let childData;
    ...
    // This should remove an item from the child query, but *not* the parent
    // query.
    ea.reset();
  ...
  it('Server: Setting grandchild of item in limit works.', async function() {
    ...
    expect(snaps).to.deep.equal(
        [ {a : {name : 'Mike'}}, {a : {name : 'Fred'}} ]);
  });

  it('Server: Updating grandchildren of item in limit works.',
     async function() {
       const refPair = getRandomNode(2), ref = refPair[0], ref2 = refPair[1];
  ...
  it('Server: Priority-only updates are processed correctly by server (1).',
     async function() {
       const refPair = getRandomNode(2), readRef = refPair[0],
             writeRef = refPair[1];
  ...
  // Same as above but with an endAt() so we hit CompoundQueryView instead of
  // SimpleLimitView.
  it('Server: Priority-only updates are processed correctly by server (2).',
     async function() {
       const refPair = getRandomNode(2), readRef = refPair[0],
             writeRef = refPair[1];
  ...
  it("Cache doesn't remove items that have fallen out of view.",
     async function() {
       const refPair = getRandomNode(2), readRef = refPair[0],
             writeRef = refPair[1];
  ...
  it('handles an update that moves another child that has a deeper listener out of view',
     async function() {
       const refs = getRandomNode(2);
       const reader = refs[0];
       const writer = refs[1];
       ...
       reader.child('b/d').on(
           'value', function(snap) { expect(snap.val()).to.equal(4); });
  ...
  it('Handles fallback for orderBy', async function() {
    ...
    ref.orderByChild('foo').on('child_added',
                               function(snap) { children.push(snap.key); });

    // Set initial data
    await ref.set({a : {foo : 3}, b : {foo : 1}, c : {foo : 2}});
  ...
  it('Snapshot children respect default ordering', function(done) {
    const refPair = getRandomNode(2);
    const queryRef = refPair[0], writerRef = refPair[1];
    ...
    writerRef.set(list, function() {
      queryRef.orderByChild('name').once('value', function(snap) {
        ...
        // Validate that snap.forEach() resets ordering to default for child
        // snaps
        const orderedNames = [];
        snap.forEach(function(childSnap) {
          ...
          expect(orderedKeys).to.deep.equal([
            'thisvaluefirst', 'name', 'thisvaluelast'
          ]);
        });
  ...
  it('Adding listens for the same paths does not check fail', function(done) {
    ...
    // default listener and one-time listener underneath. During one-time
    // listener registration, sync-tree traversal stopped as soon as it found a
    // complete server cache (this is the case for not indexed query view). The
    // problem is that the same traversal was looking for a ancestor default
    // view, and the early exit prevented from finding the default listener
    // above the one-time listener. Event removal code path wasn't removing the
    // listener because it stopped as soon as it found the default view. This
    // left the zombie one-time listener and check failed on the second attempt
