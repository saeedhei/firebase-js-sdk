...
describe('Promise Tests', function() {
  ...
  it('wraps Firebase.set', function() {
    ...
    return ref.set(5)
        ...
        .then(function(read) { expect(read.val()).to.equal(5); });
  });
  ...
  it('wraps Firebase.remove', function() {
    ...
    return ref.set({'a' : 'b'})
        ...
        .then(function() { return ref.once('value'); })
        .then(function(snap) { expect(snap.val()).to.equal(null); });
  });
  ...
  it('wraps Firebase.setWithPriority', function() {
    const ref = (getRandomNode() as Reference);
    return ref.setWithPriority('hi', 5)
        .then(function() { return ref.once('value'); })
        .then(function(snap) {
  ...
  it('wraps Firebase.transaction', function() {
    const ref = (getRandomNode() as Reference);
    return ref.transaction(function() { return 5; })
        ...
        .then(function(result) { expect(result.committed).to.equal(false); });
  });

  it('exposes catch in the return of Firebase.push', function() {
    // Catch is a pain in the bum to provide safely because "catch" is a
    // reserved word and ES3 and below require you to use quotes to define it,
    // but the closure linter really doesn't want you to do that either.
    const ref = (getRandomNode() as Reference);
    const pushed = ref.push(6);
  ...
  it('wraps onDisconnect.remove', function() {
    ...
    refInfo.once('value',
                 function(snapshot) { expect(snapshot.val()).to.equal(true); });

    return writer.child('here today')
        .set('gone tomorrow')
        .then(function() {
  ...
  it('wraps onDisconnect.setWithPriority', function() {
    ...
    const writer = refs[0];
    const reader = refs[1];
    return writer.child('meaning of life')
        .onDisconnect()
