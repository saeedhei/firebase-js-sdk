...
describe('Node Tests', function() {
  ...
  it("LeafNode.updateImmediateChild returns a new children node.", function() {
    ...
    expect((y.getImmediateChild('test') as LeafNode).getValue())
        .to.equal('foo');
  });
  ...
  it('ChildrenNode.updatePriority returns a new internal node without changing the old.',
     function() {
       const x = ChildrenNode.EMPTY_NODE.updateImmediateChild("child",
                                                              new LeafNode(5));
  ...
  it('ChildrenNode.updateImmediateChild returns a new internal node with the new child, without changing the old.',
     function() {
       ...
       const x = new ChildrenNode(children, ChildrenNode.EMPTY_NODE,
                                  IndexMap.Default);
  ...
  it("Node.hash() works correctly.", function() {
    ...
    // !!!NOTE!!! These hashes must match what the server generates.  If you
    // change anything so these hashes change, make sure you change the
    // corresponding server code.
    expect(node.getImmediateChild("intNode").hash())
        .to.equal("eVih19a6ZDz3NL32uVBtg9KSgQY=");
    expect(node.getImmediateChild("doubleNode").hash())
        .to.equal("vf1CL0tIRwXXunHcG/irRECk3lY=");
    expect(node.getImmediateChild("stringNode").hash())
        .to.equal("CUNLXWpCVoJE6z7z1vE57lGaKAU=");
    expect(node.getImmediateChild("boolNode").hash())
        .to.equal("E5z61QM0lN/U2WsOnusszCTkR8M=");
  ...
  it("Node.hash() stress...", function() {
    const node = nodeFromJSON({
      ...
      e : {'.value' : 12345678901234568, '.priority' : "🐵"},
      "✔" : "foo",
      '.priority' : "✔"
    });
    expect(node.getImmediateChild('a').hash())
        .to.equal('7HxgOBDEC92uQwhCuuvKA2rbXDA=');
    expect(node.getImmediateChild('b').hash())
        .to.equal('8R+ekVQmxs6ZWP0fdzFHxVeGnWo=');
    expect(node.getImmediateChild('c').hash())
        .to.equal('JoKoFUnbmg3/DlY70KaDWslfYPk=');
    expect(node.getImmediateChild('d').hash())
        .to.equal('Y41iC5+92GIqXfabOm33EanRI8s=');
    expect(node.getImmediateChild('e').hash())
        .to.equal('+E+Mxlqh5MhT+On05bjsZ6JaaxI=');
    expect(node.getImmediateChild('✔').hash())
        .to.equal('MRRL/+aA/uibaL//jghUpxXS/uY=');
    expect(node.hash()).to.equal('CyC0OU8GSkOAKnsPjheWtWC0Yxo=');
  });

  it("ChildrenNode.getPredecessorChild works correctly.", function() {
    const node =
        nodeFromJSON({d : true, a : true, g : true, c : true, e : true});

    // HACK: Pass null instead of the actual childNode, since it's not actually
    // needed.
    expect(node.getPredecessorChildName('a', null, DEFAULT_INDEX))
        .to.equal(null);
    expect(node.getPredecessorChildName('c', null, DEFAULT_INDEX))
        .to.equal('a');
    expect(node.getPredecessorChildName('d', null, DEFAULT_INDEX))
        .to.equal('c');
    expect(node.getPredecessorChildName('e', null, DEFAULT_INDEX))
        .to.equal('d');
    expect(node.getPredecessorChildName('g', null, DEFAULT_INDEX))
        .to.equal('e');
  });
  ...
  it("SortedChildrenNode.getPredecessorChild works correctly.", function() {
    ...
    expect(node.getPredecessorChildName('a', node.getImmediateChild('a'),
                                        DEFAULT_INDEX))
        .to.equal('c');
    expect(node.getPredecessorChildName('c', node.getImmediateChild('c'),
                                        DEFAULT_INDEX))
        .to.equal('d');
    expect(node.getPredecessorChildName('d', node.getImmediateChild('d'),
                                        DEFAULT_INDEX))
        .to.equal('e');
    expect(node.getPredecessorChildName('e', node.getImmediateChild('e'),
                                        DEFAULT_INDEX))
        .to.equal('g');
    expect(node.getPredecessorChildName('g', node.getImmediateChild('g'),
                                        DEFAULT_INDEX))
        .to.equal(null);
  });
  ...
  it("SortedChildrenNode.updateImmediateChild works correctly.", function() {
    ...
    expect((node.getImmediateChild('c') as LeafNode).getValue())
        .to.equal(false);
  ...
  it("removing nodes correctly removes intermediate nodes with no remaining children",
     function() {
       ...
       const newNode =
           node.updateChild(new Path('a/b/c'), ChildrenNode.EMPTY_NODE);
