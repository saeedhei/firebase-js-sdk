...
describe('Path Tests', function() {
  ...
  it('contains() contains the path and any child path.', function() {
    ...
    expect(new Path('/a/b/c').popFront().contains(new Path('/b/c')))
        .to.equal(true);
    expect(new Path('/a/b/c').popFront().contains(new Path('/b/c/d')))
        .to.equal(true);

    expect(new Path('/a/b/c').contains(new Path('/b/c'))).to.equal(false);
    expect(new Path('/a/b/c').contains(new Path('/a/c/b'))).to.equal(false);

    expect(new Path('/a/b/c').popFront().contains(new Path('/a/b/c')))
        .to.equal(false);
    expect(new Path('/a/b/c').popFront().contains(new Path('/b/c')))
        .to.equal(true);
    expect(new Path('/a/b/c').popFront().contains(new Path('/b/c/d')))
  ...
  it('popFront() returns the parent', function() {
    ...
    expect(new Path('/a/b/c').popFront().popFront().popFront().toString())
        .to.equal('/');
    expect(new Path('/a/b/c')
               .popFront()
               .popFront()
               .popFront()
               .popFront()
  ...
  it('parent() returns the parent', function() {
    ...
    expect(new Path('/a/b/c').parent().parent().parent().toString())
        .to.equal('/');
    expect(new Path('/a/b/c').parent().parent().parent().parent())
        .to.equal(null);
  });
