...
describe('CompoundWrite Tests', function() {
  ...
  const CHILDREN_NODE =
      nodeFromJSON({'child-1' : 'value-1', 'child-2' : 'value-2'});
  ...
  function assertNodeGetsCorrectPriority(compoundWrite, node, priority) {
    if (node.isEmpty()) {
      ...
      expect(compoundWrite.apply(node))
          .to.deep.equal(node.updatePriority(priority));
    }
  }
  ...
  it('Empty merge is empty',
     function() { expect(CompoundWrite.Empty.isEmpty()).to.be.true; });

  it('CompoundWrite with priority update is not empty.', function() {
    expect(CompoundWrite.Empty.addWrite(new Path('.priority'), PRIO_NODE)
               .isEmpty())
        .to.be.false;
  });
  ...
  it('CompoundWrite with root update is not empty.', function() {
    expect(CompoundWrite.Empty.addWrite(Path.Empty, LEAF_NODE).isEmpty())
        .to.be.false;
  });

  it('CompoundWrite with empty root update is not empty.', function() {
    expect(CompoundWrite.Empty.addWrite(Path.Empty, EMPTY_NODE).isEmpty())
        .to.be.false;
  });
  ...
  it('CompoundWrite with root priority update, child write is not empty.',
     function() {
       let compoundWrite =
           CompoundWrite.Empty.addWrite(new Path('.priority'), PRIO_NODE);
       expect(compoundWrite.childCompoundWrite(new Path('.priority')).isEmpty())
           .to.be.false;
     });
  ...
  it('shallow update removes deep update', function() {
    ...
    const updateThree =
        nodeFromJSON({'foo' : 'foo-value', 'bar' : 'bar-value'});
    ...
    const expectedChildOne = {'foo' : 'foo-value', 'bar' : 'bar-value'};
    const expected = CHILDREN_NODE.updateImmediateChild(
        'child-1', nodeFromJSON(expectedChildOne));
  ...
  it('child priority updates empty priority on child write', function() {
    let compoundWrite = CompoundWrite.Empty;
    compoundWrite =
        compoundWrite.addWrite(new Path('child-1/.priority'), EMPTY_NODE);
    const node = new LeafNode('foo', PRIO_NODE);
    assertNodeGetsCorrectPriority(
        compoundWrite.childCompoundWrite(new Path('child-1')), node,
        EMPTY_NODE);
  });
  ...
  it('deep priority set works on empty node when other set is available',
     function() {
       let compoundWrite = CompoundWrite.Empty;
       compoundWrite =
           compoundWrite.addWrite(new Path('foo/.priority'), PRIO_NODE);
       compoundWrite = compoundWrite.addWrite(new Path('foo/child'), LEAF_NODE);
       const node = compoundWrite.apply(EMPTY_NODE);
       assertNodesEqual(PRIO_NODE,
                        node.getChild(new Path('foo')).getPriority());
     });
  ...
  it('child merge removes node on deeper paths', function() {
    ...
    assertNodesEqual(
        EMPTY_NODE,
        compoundWrite.childCompoundWrite(new Path('foo/not/existing'))
            .apply(LEAF_NODE));
  });
  ...
  it('child merge with empty path is same merge', function() {
    ...
    const update = nodeFromJSON({'foo' : 'foo-value', 'bar' : 'bar-value'});
    compoundWrite = compoundWrite.addWrite(Path.Empty, update);
    expect(compoundWrite.childCompoundWrite(Path.Empty))
  ...
  it('deep update removes priority there', function() {
    ...
    compoundWrite =
        compoundWrite.addWrite(new Path('foo/.priority'), PRIO_NODE);
    compoundWrite =
        compoundWrite.addWrite(new Path('foo'), nodeFromJSON('bar'));
  ...
  it('adding updates at root works', function() {
    ...
    const expected = {'child-1' : 'new-value-1', 'child-3' : 'value-3'};
    assertNodesEqual(nodeFromJSON(expected),
  ...
  it('child write of root priority works', function() {
    let compoundWrite =
        CompoundWrite.Empty.addWrite(new Path('.priority'), PRIO_NODE);
    assertNodesEqual(PRIO_NODE,
                     compoundWrite.childCompoundWrite(new Path('.priority'))
                         .apply(EMPTY_NODE));
  });
  ...
  it('complete children only returns complete overwrites', function() {
    ...
    expect(compoundWrite.getCompleteChildren()).to.deep.equal([ new NamedNode(
        'child-1', LEAF_NODE) ]);
  });
  ...
  it('complete children doesnt return deep overwrites', function() {
    let compoundWrite = CompoundWrite.Empty;
    compoundWrite =
        compoundWrite.addWrite(new Path('child-1/deep/path'), LEAF_NODE);
  ...
  it('complete children return all complete children but no incomplete',
     function() {
       ...
       const expected = {'child-2' : LEAF_NODE, 'child-3' : EMPTY_NODE};
       const actual = {};
  ...
  it('compound write with deep update has shadowing write', function() {
    let compoundWrite =
        CompoundWrite.Empty.addWrite(new Path('deep/update'), LEAF_NODE);
  ...
  it('deep removes has no effect on overlaying set', function() {
    ...
    compoundWrite = compoundWrite.addWrite(new Path('child-1/baz'), updateTwo);
    compoundWrite =
        compoundWrite.addWrite(new Path('child-1/foo'), updateThree);
  ...
  it('remove at root also removes priority', function() {
    let compoundWrite = CompoundWrite.Empty;
    compoundWrite =
        compoundWrite.addWrite(Path.Empty, new LeafNode('foo', PRIO_NODE));
  ...
  it('updating priority doesnt overwrite leaf node', function() {
    ...
    compoundWrite = compoundWrite.addWrite(Path.Empty, LEAF_NODE);
    compoundWrite =
        compoundWrite.addWrite(new Path('child/.priority'), PRIO_NODE);
  ...
  it('Overwrites existing child', function() {
    ...
    compoundWrite = compoundWrite.addWrite(path, LEAF_NODE);
    expect(compoundWrite.apply(CHILDREN_NODE))
        .to.deep.equal(
  ...
  it('Updates existing child', function() {
    ...
    compoundWrite = compoundWrite.addWrite(path, LEAF_NODE);
    expect(compoundWrite.apply(CHILDREN_NODE))
        .to.deep.equal(CHILDREN_NODE.updateChild(path, LEAF_NODE));
  });
  ...
  it('Updates priority of child', function() {
    ...
    assertNodesEqual(CHILDREN_NODE.updateChild(path, PRIO_NODE),
                     compoundWrite.apply(CHILDREN_NODE));
  });
  ...
  it("child priority doesn't update empty node priority on child merge",
     function() {
       let compoundWrite = CompoundWrite.Empty;
       compoundWrite =
           compoundWrite.addWrite(new Path('child-1/.priority'), PRIO_NODE);
       assertNodeGetsCorrectPriority(
           compoundWrite.childCompoundWrite(new Path('child-1')), EMPTY_NODE,
           EMPTY_NODE);
     });
