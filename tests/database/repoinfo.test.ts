...
import {
  ...
  VERSION_PARAM,
  WEBSOCKET
} from '../../src/database/realtime/Constants';
...
describe('RepoInfo', function() {
  it('should return the correct URL', function() {
    ...
    expect(websocketUrl)
        .to.equal('wss://test-ns.firebaseio.com/.ws?v=5&ls=test');

    const longPollingUrl = repoInfo.connectionURL(LONG_POLLING, urlParams);
    expect(longPollingUrl)
        .to.equal('https://test-ns.firebaseio.com/.lp?v=5&ls=test');
  });
});
