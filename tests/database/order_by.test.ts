...
describe('.orderBy tests', function() {
  ...
  it('Snapshots are iterated in order', function() {
    ...
    orderedRef.on('value', function(snap) {
      snap.forEach(function(childSnap) { valueOrder.push(childSnap.key); });
    });
  ...
  it('Snapshots are iterated in order for value', function() {
    ...
    const initial = {alex : 60, rob : 56, vassili : 55.5, tony : 52, greg : 52};
  ...
  it('child_added events are in the correct order', function() {
    ...
    const initial = {a : {value : 5}, c : {value : 3}};

    const added = [];
    ref.orderByChild('value').on('child_added',
                                 function(snap) { added.push(snap.key); });
    ref.set(initial);

    expect(added).to.deep.equal([ 'c', 'a' ]);

    ref.update({b : {value : 4}, d : {value : 2}});
  ...
  it('Server respects KeyIndex', function(done) {
    ...
    writer.set(initial, function() {
      orderedRef.on('value', function(snap) {
        snap.forEach(function(childSnap) { actual.push(childSnap.key); });
  ...
  it('Removing default listener removes non-default listener that loads all data',
     function(done) {
       ...
       ref.set(initial, function(err) {
         ...
         ref.on('value', function() {});
         // Should remove both listener and should remove the listen sent to the
         // server
         ref.off();

         // This used to crash because a listener for ref.orderByKey() existed
         // already
