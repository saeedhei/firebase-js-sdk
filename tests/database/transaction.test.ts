...
describe('Transaction Tests', function() {
  it('New value is immediately visible.', function() {
    ...
    node.child('foo').transaction(function() { return 42; });

    let val = null;
    node.child('foo').on('value', function(snap) { val = snap.val(); });
  ...
  it('Non-aborted transaction sets committed to true in callback.',
     function(done) {
       ...
       node.transaction(function() { return 42; },
  ...
  it('Use transaction to create a node, make sure exactly one event is received.',
     function() {
       ...
       node.child('a').transaction(function() { return 42; },
                                   function() {
                                     done = true;
                                     ea.addEvent();
                                   });
  ...
  it('Second transaction gets run immediately on previous output and only runs once.',
     function(done) {
       const nodePair = (getRandomNode(2) as Reference[]);
       let firstRun = false, firstDone = false, secondRun = false,
           secondDone = false;
  ...
  it('Set() cancels pending transactions and re-runs affected transactions.',
     async function() {
       // We do 3 transactions: 1) At /foo, 2) At /, and 3) At /bar.
       // Only #1 is sent to the server immediately (since 2 depends on 1 and 3
       // depends on 2). We set /foo to 0.
       //   Transaction #1 should complete as planned (since it was already
       //   sent). Transaction #2 should be aborted by the set. Transaction #3
       //   should be re-run after #2 is reverted, and then be sent to the
       //   server and succeed.
       let firstDone = false, secondDone = false, thirdDone = false;
       const node = (getRandomNode() as Reference);
       let nodeSnap = null;
       ...
       let firstRun = false, secondRun = false, thirdRunCount = 0;
       ...
       // third. However, a new value event won't be triggered until the
       // listener is complete, so we're left with the last value event
       node.child('foo').set(0);

       expect(firstDone).to.equal(false);
       expect(secondDone).to.equal(true);
       expect(thirdRunCount).to.equal(2);
       // Note that the set actually raises two events, one overlaid on top of
       // the original transaction value, and a second one with the re-run value
  ...
  it('Priority is preserved when setting data.', async function() {
    ...
    let promise = node.transaction(function() { return 'new value' },
  ...
  it('Tetris bug test - Can do transactions from transaction callback.',
     async function() {
       const nodePair = (getRandomNode(2) as Reference[]), writeDone = false;
  ...
  it('Transaction aborts after 25 retries.', function(done) {
    const restoreHash = hijackHash(function() { return 'duck, duck, goose.'; });
  ...
  it('Set should cancel already sent transactions that come back as datastale.',
     function(done) {
       ...
       nodePair[0].set(5, function() {
         ...
         // Transaction should get sent but fail due to stale data, and then
         // aborted because of the below set().
         nodePair[1].set(32);
       });
     });
  ...
  it('Update should not cancel unrelated transactions', async function() {
    ...
    const restoreHash = hijackHash(function() { return 'foobar'; });

    await node.child('foo').set(5);

    // 'foo' gets overwritten in the update so the transaction gets cancelled.
    node.child('foo').transaction(function(old) { return 72; },
                                  function(error, committed, snapshot) {
                                    expect(error.message).to.equal('set');
                                    expect(committed).to.equal(false);
    ...
    // 'bar' does not get touched during the update and the transaction
    // succeeds.
    node.child('bar').transaction(function(old) { return 72; },
                                  function(error, committed, snapshot) {
                                    expect(error).to.equal(null);
                                    expect(committed).to.equal(true);
  ...
  it('Test immediately aborted transaction.', function(done) {
    const node = (getRandomNode() as Reference);
    // without callback.
    node.transaction(function(curr) { return; });
  ...
  it('Merged transactions have correct snapshot in onComplete.',
     async function() {
       const nodePair = (getRandomNode(2) as Reference[]), node1 = nodePair[0],
             node2 = nodePair[1];
       ...
       const tx1 = node2.transaction(
           ...
           function(error, committed, snapshot) {
             ...
             // Per new behavior, will include the accepted value of the
       ...
       const tx2 = node2.child('a').transaction(
           function(val) {
             if (val !== null) {
               expect(val).to.equal(
                   1); // should run after the first transaction.
             }
             return 2;
           },
           function(error, committed, snapshot) {
     ...
     function(done) {
       ...
       node.transaction(
           ...
           function() {
             ...
             callbackCalled = true;
             node.set('transaction done', function() { done(); });
           });
     });
  ...
  it('Doing set() in aborted transaction callback works. Case 870.',
     function(done) {
       ...
       node1.set('initial', function() {
         ...
         node2.transaction(
             function(val) {
               // Return dummy value until we're called with the actual current
               // value.
               if (val === null)
                 return 'hi';
             ...
             function(error, committed, snapshot) {
               expect(callbackCalled).to.not.be.ok;
               callbackCalled = true;
               node2.set('transaction done', function() { done(); });
             });
       });
     });
  ...
  it('Pending transactions are canceled on disconnect.', function(done) {
    ...
    ref.set('initial', function() {
      ref.transaction(function(current) { return 'new'; },
                      function(error, committed, snapshot) {
                        expect(committed).to.equal(false);
      ...
      // Kill the connection, which should cancel the outstanding transaction,
      // since we don't know if it was committed on the server or not.
  ...
  it('Transaction without local events (1)', async function() {
    ...
    ref.transaction(function() { return 'hello!'; },
                    function(error, committed, snapshot) {
                      expect(error).to.be.null;
                      expect(committed).to.equal(true);
                      expect(snapshot.val()).to.equal('hello!');
    ...
    expect(actions).to.deep.equal(
        [ 'value null', 'txn run', 'value hello!', 'txn completed' ]);
  });

  // This test is meant to ensure that with applyLocally=false, while the
  // transaction is outstanding, we continue to get events from other clients.
  it('Transaction without local events (2)', function(done) {
    const refPair = (getRandomNode(2) as Reference[]), ref1 = refPair[0],
          ref2 = refPair[1];
    const restoreHash = hijackHash(function() { return 'badhash'; });
    const SETS = 4;
    let events = [], retries = 0, setsDone = 0;

    function txn1(next) {
      // Do a transaction on the first connection which will keep retrying
      // (cause we hijacked the hash). Make sure we're getting events for the
      // sets happening on the second connection.
      ref1.transaction(
          function(current) {
            retries++;
            // We should be getting server events while the transaction is
            // outstanding.
  ...
  it('Transaction runs on null only once after reconnect (Case 1981).',
     async function() {
  ...
  // Provided by bk@thinkloop.com, this was failing when we sent puts before
  // listens, but passes now.
  ...
  it('Transaction with integer keys', function(done) {
    ...
    ref.set({1 : 1, 5 : 5, 10 : 10, 20 : 20}, function() {
      ref.transaction(function(current) { return 42; },
                      function(error, committed) {
                        expect(error).to.be.null;
                        expect(committed).to.equal(true);
                        done();
  ...
  it('Return null from first run of transaction.', function(done) {
    const ref = (getRandomNode() as Reference);
    ref.transaction(function(c) { return null; },
                    function(error, committed) {
                      expect(error).to.equal(null);
                      expect(committed).to.equal(true);
                      done();
  ...
  it('Bubble-app transaction bug.', function(done) {
    const ref = (getRandomNode() as Reference);
    ref.child('a').transaction(function() { return 1; });
    ref.child('a').transaction(function(current) { return current + 42; });
    ref.child('b').transaction(function() { return 7; });
    ref.transaction(
        function(current) {
  ...
  it('Transaction and priority: Can set priority in transaction on empty node',
     async function() {
       const ref = (getRandomNode() as Reference);
  ...
  it("Transaction and priority: Transaction doesn't change priority.",
     async function() {
       const ref = (getRandomNode() as Reference);

       await ref.set({'.value' : 42, '.priority' : 7});

       await ref.transaction(function(current) { return 12; });
  ...
  it('Transaction and priority: Transaction can change priority on non-empty node.',
     async function() {
       const ref = (getRandomNode() as Reference);
  ...
  it('Transaction and priority: Changing priority on siblings.',
     async function() {
       const ref = (getRandomNode() as Reference);
  ...
  it('Transaction and priority: Leaving priority on siblings.',
     async function() {
       const ref = (getRandomNode() as Reference);
       ...
       const tx1 =
           ref.child('a').transaction(function(current) { return 'a2'; });

       const tx2 =
           ref.child('b').transaction(function(current) { return 'b2'; });
  ...
  it('transaction() doesn\'t pick up cached data from previous once().',
     function(done) {
       ...
       const me = refPair[0], other = refPair[1];
  ...
  it("server values: local timestamp should eventually (but not immediately) match the server with txns",
     function(done) {
       const refPair = (getRandomNode(2) as Reference[]), writer = refPair[0],
             reader = refPair[1], readSnaps = [], writeSnaps = [];

       const evaluateCompletionCriteria = function() {
         if (readSnaps.length === 1 && writeSnaps.length === 2) {
           expect(Math.abs(new Date().getTime() - writeSnaps[0].val()) < 10000)
               .to.equal(true);
           expect(Math.abs(new Date().getTime() - writeSnaps[0].getPriority()) <
                  10000)
               .to.equal(true);
           expect(Math.abs(new Date().getTime() - writeSnaps[1].val()) < 10000)
               .to.equal(true);
           expect(Math.abs(new Date().getTime() - writeSnaps[1].getPriority()) <
                  10000)
               .to.equal(true);

           expect(writeSnaps[0].val() === writeSnaps[1].val()).to.equal(false);
           expect(writeSnaps[0].getPriority() === writeSnaps[1].getPriority())
               .to.equal(false);
           expect(writeSnaps[1].val() === readSnaps[0].val()).to.equal(true);
           expect(writeSnaps[1].getPriority() === readSnaps[0].getPriority())
               .to.equal(true);
           done();
         }
       };
       ...
       // Generate the server value offline to make sure there's a time gap
       // between the client's guess of the timestamp and the server's actual
       // timestamp.
  ...
  it("transaction() still works when there's a query listen.", function(done) {
    ...
    ref.set({a : 1, b : 2}, function() {
      ref.limitToFirst(1).on('child_added', function() {});

      ref.child('a').transaction(function(current) { return current; },
                                 function(error, committed, snapshot) {
                                   expect(error).to.equal(null);
                                   expect(committed).to.equal(true);
                                   if (!error) {
  ...
  it('transactions raise correct child_changed events on queries',
     async function() {
       const ref = (getRandomNode() as Reference);
       ...
       query.on('child_added',
                function(snapshot) { snapshots.push(snapshot); });

       query.on('child_changed',
                function(snapshot) { snapshots.push(snapshot); });

       await ref.child('foo').transaction(function(
                                              current) { return {value : 2}; },
                                          function(error, committed, snapshot) {
                                            expect(error).to.equal(null);
                                            expect(committed).to.equal(true);
                                          },
  ...
  it('out of order remove writes are handled correctly', function(done) {
    ...
    ref.set({foo : 'bar'});
    ref.transaction(function() { return 'transaction-1'; }, function() {});
    ref.transaction(function() { return 'transaction-2'; }, function() {});
