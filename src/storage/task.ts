...
export class UploadTask {
   ...
   */
  constructor(ref: Reference, authWrapper: AuthWrapper, location: Location,
              mappings: fbsMetadata.Mappings, blob: FbsBlob,
  ...
  private makeProgressCallback_(): (p1: number, p2: number) => void {
    ...
    return (loaded, total) => { this.updateProgress_(sizeBefore + loaded); };
  }
  ...
  private createResumable_() {
    this.resolveToken_((authToken) => {
      const requestInfo = fbsRequests.createResumableUpload(
          ...
          this.metadata_);
      const createRequest =
          this.authWrapper_.makeRequest(requestInfo, authToken);
  ...
  private continueUpload_() {
    ...
    const status = new fbsRequests.ResumableUploadStatus(this.transferred_,
                                                         this.blob_.size());
  ...
  get snapshot(): UploadTaskSnapshot {
    ...
    return new UploadTaskSnapshot(this.transferred_, this.blob_.size(),
                                  externalState, this.metadata_, this,
                                  this.ref_);
  }
  ...
  on(type: TaskEvent, nextOrObserver = undefined, error = undefined,
    ...
    function nextOrObserverValidator(p: any) {
      ...
      try {
        observerValidator(p);
        const anyDefined = typeUtils.isJustDef(p['next']) ||
    ...
    function makeBinder(specs: ArgSpec[]|null): Subscribe<UploadTaskSnapshot> {
      function binder(nextOrObserver: NextFn<UploadTaskSnapshot>|
                      {[name: string] : string | null}|null,
                      error?: ErrorFn|null, opt_complete?: CompleteFn|null) {
        if (specs !== null) {
          fbsArgs.validate('on', specs, arguments);
        }
        const observer = new Observer(nextOrObserver, error, completed);
        self.addObserver_(observer);
        return () => { self.removeObserver_(observer); };
      }
      return binder;
    }
   ...
   */
  then<U>(onFulfilled
          ?: ((value: UploadTaskSnapshot) => U | PromiseLike<U>)|null,
          onRejected?: ((error: any) => U | PromiseLike<U>)|null): Promise<U> {
  ...
  private notifyObservers_() {
    ...
    observers.forEach((observer) => { this.notifyObserver_(observer); });
  }
