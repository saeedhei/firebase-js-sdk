...
export function dataFromString(format: StringFormat,
      ...
      if (c <= 2047) {
        ...
        if ((c & 64512) == 55296) {
          ...
          if (!valid) {
            ...
            b.push(240 | c >> 18, 128 | c >> 12 & 63, 128 | c >> 6 & 63,
                   128 | c & 63);
          }
        } else {
          if ((c & 64512) == 56320) {
...
export function percentEncodedBytes_(string: string): Uint8Array {
  ...
  try {
    ...
    throw errorsExports.invalidFormat(StringFormat.DATA_URL,
                                      'Malformed data URL.');
  }
...
export function base64Bytes_(format: StringFormat, string: string): Uint8Array {
  ...
  case StringFormat.BASE64: {
    ...
    if (hasMinus || hasUnder) {
      ...
      throw errorsExports.invalidFormat(
          format, 'Invalid character \'' + invalidChar +
  ...
  case StringFormat.BASE64URL: {
    ...
    if (hasPlus || hasSlash) {
      ...
      throw errorsExports.invalidFormat(format,
...
class DataURLParts {
  ...
  constructor(dataURL: string) {
    ...
    if (middle != null) {
      ...
      this.contentType =
          this.base64 ? middle.substring(0, middle.length - ';base64'.length)
                      : middle;
    }
