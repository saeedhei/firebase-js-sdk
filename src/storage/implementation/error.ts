...
export class FirebaseStorageError implements Error {
  ...
  codeProp(): string { return this.code; }

  codeEquals(code: Code): boolean {
    return prependCode(code) === this.codeProp();
  }

  serverResponseProp(): string|null { return this.serverResponse_; }
  ...
  get code(): string { return this.code_; }

  get message(): string { return this.message_; }

  get serverResponse(): null|string { return this.serverResponse_; }
}
...
export function prependCode(code: Code): string { return 'storage/' + code; }
...
export function objectNotFound(path: string): FirebaseStorageError {
  return new FirebaseStorageError(Code.OBJECT_NOT_FOUND,
                                  'Object \'' + path + '\' does not exist.');
}

export function bucketNotFound(bucket: string): FirebaseStorageError {
  return new FirebaseStorageError(Code.BUCKET_NOT_FOUND,
...
export function quotaExceeded(bucket: string): FirebaseStorageError {
  return new FirebaseStorageError(Code.QUOTA_EXCEEDED,
                                  'Quota for bucket \'' + bucket +
...
export function unauthorized(path: string): FirebaseStorageError {
  return new FirebaseStorageError(Code.UNAUTHORIZED,
...
export function invalidChecksum(path: string, checksum: string,
                                calculated: string): FirebaseStorageError {
  return new FirebaseStorageError(
      Code.INVALID_CHECKSUM, 'Uploaded/downloaded object \'' + path +
...
export function invalidArgument(index: number, fnName: string,
                                message: string): FirebaseStorageError {
  return new FirebaseStorageError(Code.INVALID_ARGUMENT,
                                  'Invalid argument in `' + fnName +
                                      '` at index ' + index + ': ' + message);
}

export function invalidArgumentCount(argMin: number, argMax: number,
                                     fnName: string,
                                     real: number): FirebaseStorageError {
  let countPart;
  let plural;
  if (argMin === argMax) {
    countPart = argMin;
  ...
  return new FirebaseStorageError(Code.INVALID_ARGUMENT_COUNT,
                                  'Invalid argument count in `' + fnName +
                                      '`: Expected ' + countPart + ' ' +
                                      plural + ', received ' + real + '.');
}

export function appDeleted(): FirebaseStorageError {
  return new FirebaseStorageError(Code.APP_DELETED,
 ...
 */
export function invalidFormat(format: string,
                              message: string): FirebaseStorageError {
  return new FirebaseStorageError(Code.INVALID_FORMAT,
...
export function internalError(message: string): FirebaseStorageError {
  throw new FirebaseStorageError(Code.INTERNAL_ERROR,
