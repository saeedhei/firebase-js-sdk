 ...
 */
import {deepCopy, deepExtend, patchProperty} from '../utils/deep_copy';
...
export type FirebaseOptions = {
  ...
  messagingSenderId?: string [name: string] : any
};
...
export interface FirebaseNamespace {
  ...
  INTERNAL: {
     ...
     */
    registerService(name: string, createService: FirebaseServiceFactory,
                    serviceProperties?: {[prop: string] : any},
                    appHook?: AppHook, allowMultipleInstances?: boolean) :
     ...
     */
    createSubscribe<T>(executor: (observer: Observer<T>) => void,
...
class FirebaseAppImpl implements FirebaseApp {
  ...
  private services_:
      {[name: string] : {[serviceName: string] : FirebaseService}} = {};
  ...
  constructor(options: FirebaseOptions, name: string,
    ...
    this.INTERNAL = {
      ...
      'addAuthTokenListener' : (callback: (token: string|null) => void) => {
        ...
        // Make sure callback is called, asynchronously, in the absence of the
        // auth module
        setTimeout(() => callback(null), 0);
      },
      'removeAuthTokenListener' : (callback) => {
        tokenListeners =
            tokenListeners.filter(listener => listener !== callback);
      },
    };
  }
  ...
  delete(): Promise<void> {
    return new PromiseImpl((resolve) => {
        ...
        .then(() => {
          ...
          return PromiseImpl.all(services.map(
              (service) => { return service.INTERNAL!.delete(); }));
        })
        .then((): void => {
   ...
   * @internal
   */
  _getService(name: string, instanceIdentifier: string = DEFAULT_ENTRY_NAME):
      FirebaseService {
    this.checkDestroyed_();
  ...
  private extendApp(props: {[name: string] : any}): void {
    ...
    if (props.INTERNAL && props.INTERNAL.addAuthTokenListener) {
      tokenListeners.forEach(
          listener => { this.INTERNAL.addAuthTokenListener(listener); });
      tokenListeners = [];
    }
  }
...
FirebaseAppImpl.prototype.name && FirebaseAppImpl.prototype.options ||
    FirebaseAppImpl.prototype.delete || console.log("dc");
...
export function createFirebaseNamespace(): FirebaseNamespace {
  ...
  function initializeApp(options: FirebaseOptions, name?: string): FirebaseApp {
    ...
    let app =
        new FirebaseAppImpl(options, name!, namespace as FirebaseNamespace);
   ...
   */
  function registerService(name: string, createService: FirebaseServiceFactory,
                           serviceProperties?: {[prop: string] : any},
                           appHook?: AppHook, allowMultipleInstances?: boolean):
      FirebaseServiceNamespace<FirebaseService> {
    ...
    if (appHook) {
      ...
      getApps().forEach(app => { appHook('create', app); });
    }
