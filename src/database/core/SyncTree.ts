...
export interface ListenProvider {
  startListening(query: Query, tag: number|null, hashFn: () => string,
 ...
 * and removeEventRegistration()). - Applying and caching data changes for user
 * set(), transaction(), and update() calls (applyUserOverwrite(),
 * applyUserMerge()). - Applying and caching data changes for server data
 * changes (applyServerOverwrite(), applyServerMerge()). - Generating
 * user-facing events for server and user changes (all of the apply* methods
...
export class SyncTree {
  ...
  /**
   * @param {!ListenProvider} listenProvider_ Used by SyncTree to start / stop
   * listening to server data.
   */
   ...
   */
  applyUserOverwrite(path: Path, newData: Node, writeId: number,
   ...
   * @return {!Array.<!Event>} Events to raise.
   */
  applyUserMerge(path: Path, changedChildren: {[k: string] : Node},
   ...
   * applyUserOverwrite() or applyUserMerge().
   *
   * @param {!number} writeId
   * @param {boolean=} revert True if the given write failed and needs to be
   * reverted
   * @return {!Array.<!Event>} Events to raise.
   */
   ...
   * @return {!Array.<!Event>} Events to raise.
   */
  applyTaggedQueryMerge(path: Path, changedChildren: {[k: string] : Node},
                        tag: number): Event[] {
    const queryKey = this.queryKeyForTag_(tag);
    if (queryKey) {
      const r = SyncTree.parseQueryKey_(queryKey);
  ...
  applyTaggedListenComplete(path: Path, tag: number): Event[] {
    ...
    if (queryKey) {
      ...
      const op = new ListenComplete(
          OperationSource.forServerTaggedQuery(queryId), relativePath);
      return this.applyTaggedOperation_(queryPath, op);
    } else {
      // We've already removed the query. No big deal, ignore the update
      return [];
   ...
   * @return {!Array.<!Event>} Events to raise.
   */
  addEventRegistration(query: Query,
    ...
    // need to find is the server cache. Consider optimizing this once there's a
    // better understanding of what actual behavior will be.
    ...
    if (serverCache != null) {
      ...
      subtree.foreachChild(function(childName, childSyncPoint) {
        ...
        if (completeCache) {
          serverCache =
              serverCache.updateImmediateChild(childName, completeCache);
        }
      });
    }
    ...
    let events =
        syncPoint.addEventRegistration(query, eventRegistration, writesCache,
                                       serverCache, serverCacheComplete);
  ...
  removeEventRegistration(query: Query,
    ...
    if (maybeSyncPoint && (query.queryIdentifier() === 'default' ||
                           maybeSyncPoint.viewExistsForQuery(query))) {
      /**
       * @type {{removed: !Array.<!Query>, events: !Array.<!Event>}}
       */
      const removedAndEvents = maybeSyncPoint.removeEventRegistration(
          query, eventRegistration, cancelError);
        ...
        if (!subtree.isEmpty()) {
          ...
          for (let i = 0; i < newViews.length; ++i) {
            ...
            this.listenProvider_.startListening(
                ...
                this.tagForQuery_(newQuery), listener.hashFn,
                listener.onComplete);
          }
        } else {
      ...
      if (!covered && removed.length > 0 && !cancelError) {
        ...
        if (removingDefault) {
          ...
          this.listenProvider_.stopListening(SyncTree.queryForListening_(query),
                                             defaultTag);
        } else {
          removed.forEach((queryToRemove: Query) => {
            const tagToRemove =
                this.queryToTagMap_[SyncTree.makeQueryKey_(queryToRemove)];
            this.listenProvider_.stopListening(
                SyncTree.queryForListening_(queryToRemove), tagToRemove);
          });
        }
      }
  ...
  calcCompleteEventCache(path: Path, writeIdsToExclude?: number[]): Node|null {
    ...
    const serverCache =
        this.syncPointTree_.findOnPath(path, function(pathSoFar, syncPoint) {
   ...
   * @private
   */
  private collectDistinctViewsForSubTree_(subtree: ImmutableTree<SyncPoint>):
      View[] {
  ...
  private static queryForListening_(query: Query): Query {
    if (query.getQueryParams().loadsAllData() &&
      ...
      // Cast is necessary because ref() technically returns Firebase which is
      // actually fb.api.Firebase which inherits from Query
   ...
   * @param {!Query} query
   * @param {!View} view
   * @return {!Array.<!Event>} This method can return events to support
  ...
  private setupListener_(query: Query, view: View): Event[] {
    ...
    const events = this.listenProvider_.startListening(
        SyncTree.queryForListening_(query), tag, listener.hashFn,
        listener.onComplete);
    ...
    // as well.
    if (tag) {
      assert(!subtree.value.hasCompleteView(),
      ...
      const queriesToStop = subtree.fold<Query[]>(function(
          relativePath, maybeChildSyncPoint, childMap) {
        if (!relativePath.isEmpty() && maybeChildSyncPoint &&
          ...
          let queries: Query[] = [];
          if (maybeChildSyncPoint) {
            queries = queries.concat(maybeChildSyncPoint.getQueryViews().map(
                view => view.getQuery()));
          }
   ...
   * @private
   */
  private createListenerForView_(view: View):
    ...
    return {
      ...
      onComplete : (status: string) : Event[] => {
        if (status === 'ok') {
          ...
          // If a listen failed, kill all of the listeners here, not just the
          // one that triggered the error. Note that this may need to be scoped
          // to just this listener if we change permissions on filtered children
  ...
  /**
   * Given a query, computes a "queryKey" suitable for use in our
   * queryToTagMap_.
   * @private
   * @param {!Query} query
   * @return {string}
  ...
  /**
   * Given a queryKey (created by makeQueryKey), parse it back into a path and
   * queryId.
   * @private
   * @param {!string} queryKey
   * @return {{queryId: !string, path: !Path}}
   */
  private static parseQueryKey_(queryKey: string):
      {queryId : string, path : Path} {
    const splitIndex = queryKey.indexOf('$');
    assert(splitIndex !== -1 && splitIndex < queryKey.length - 1,
           'Bad queryKey.');
    return {
   ...
   * @private
   */
  private static getNextQueryTag_(): number { return SyncTree.nextQueryTag_++; }
   ...
   * @private
   */
  private applyTaggedOperation_(queryPath: Path,
  ...
  /**
   * A helper method that visits all descendant and ancestor SyncPoints,
   ...
   *   1. A version of the Operation that has been made relative to the
   SyncPoint location. *   2. A WriteTreeRef of any writes we have cached at the
   SyncPoint location. *   3. A snapshot Node with cached server data, if we
   have it.

   * - We concatenate all of the events returned by each SyncPoint and return
   the result.
   *
   * @param {!Operation} operation
   * @return {!Array.<!Event>}
  ...
  private applyOperationToSyncPoints_(operation: Operation): Event[] {
    return this.applyOperationHelper_(
        operation, this.syncPointTree_, /*serverCache=*/null,
   ...
   * @return {!Array.<!Event>}
   */
  private applyOperationHelper_(operation: Operation,
                                syncPointTree: ImmutableTree<SyncPoint>,
                                serverCache: Node|null,
                                writesCache: WriteTreeRef): Event[] {

    if (operation.path.isEmpty()) {
      return this.applyOperationDescendantsHelper_(operation, syncPointTree,
      ...
      if (childTree && childOperation) {
        const childServerCache =
            serverCache ? serverCache.getImmediateChild(childName) : null;
        const childWritesCache = writesCache.child(childName);
        events = events.concat(this.applyOperationHelper_(
  ...
  private applyOperationDescendantsHelper_(
      operation: Operation, syncPointTree: ImmutableTree<SyncPoint>,
    ...
    syncPointTree.children.inorderTraversal((childName, childTree) => {
      ...
      const childOperation = operation.operationForChild(childName);
      if (childOperation) {
        events = events.concat(this.applyOperationDescendantsHelper_(
            childOperation, childTree, childServerCache, childWritesCache));
      }
    });
