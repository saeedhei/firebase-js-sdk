...
// TODO: This is pretty messy.  Ideally, a lot of this would move into
// FirebaseData, or a transaction-specific component used by FirebaseData, but
// it has ties to user callbacks (transaction update and onComplete) as well as
// the realtime connection (to send transactions to the server).  So that all
// needs to be decoupled first. For now it's part of Repo, but in its own file.

/**
 * @enum {number}
 */
export enum TransactionStatus {
  // We've run the transaction and updated transactionResultData_ with the
  // result, but it isn't currently sent to the
  // server. A transaction will go from RUN -> SENT -> RUN if it comes back from
  // the server as rejected due to
  // mismatched hash.
  RUN,

  // We've run the transaction and sent it to the server and it's currently
  // outstanding (hasn't come back as accepted
  // or rejected yet).
  SENT,

  // Temporary state used to mark completed transactions (whether successful or
  // aborted).  The transaction will be
  // removed when we get a chance to prune completed ones.
  COMPLETED,

  // Used when an already-sent transaction needs to be aborted (e.g. due to a
  // conflicting set() call that was made).
...
declare module './Repo' {
  interface Repo {
    startTransaction(path: Path, transactionUpdate: (a: any) => void,
                     onComplete:
                         ((a: Error, b: boolean, c: DataSnapshot) => void)|null,
 ...
 * @param {?function(?Error, boolean, ?DataSnapshot)} onComplete Completion
 * callback.
 * @param {boolean} applyLocally Whether or not to make intermediate results
 * visible
 */
Repo.prototype.startTransaction = function(
    path: Path, transactionUpdate: (a: any) => any,
  ...
  if (newVal === undefined) {
    ...
    if (transaction.onComplete) {
      ...
      const snapshot = new DataSnapshot(transaction.currentInputSnapshot,
                                        new Reference(this, transaction.path),
                                        PRIORITY_INDEX);
      transaction.onComplete(null, false, snapshot);
    }
  } else {
    validateFirebaseData('transaction failed: Data returned ', newVal,
                         transaction.path);
    ...
    // Note: We intentionally raise events after updating all of our transaction
    // state, since the user could start new transactions from the event
    // callbacks.
    let priorityForNode;
    ...
    const events = this.serverSyncTree_.applyUserOverwrite(
        path, newNode, transaction.currentWriteId, transaction.applyLocally);
 ...
 * @private
 */
(Repo.prototype as any).getLatestState_ = function(path: Path, excludeSets
                                                   ?: number[]): Node {
 ...
 *
 * @param {Tree.<Array.<Transaction>>=} node  transactionQueueTree node to start
 * at.
 * @private
 */
(Repo.prototype as any).sendReadyTransactions_ = function(
  ...
  if (node.getValue() !== null) {
    ...
    const allRun =
        queue.every((transaction: Transaction) =>
                        transaction.status === TransactionStatus.RUN);
    ...
    node.forEachChild(
        (childNode) => { this.sendReadyTransactions_(childNode); });
  }
};
 ...
 * @param {!Array.<Transaction>} queue Queue of transactions under the specified
 * location.
 * @private
 */
(Repo.prototype as any).sendTransactionQueue_ = function(
    path: Path, queue: Array<Transaction>) {
  ...
  for (let i = 0; i < queue.length; i++) {
    ...
    snapToSend = snapToSend.updateChild(
        relativePath, /**@type {!Node} */ (txn.currentOutputSnapshotRaw));
  }
  ...
  this.server_.put(pathToSend.toString(), dataToSend, (status: string) => {
    this.log_('transaction put response',
              {path : pathToSend.toString(), status});
    ...
    if (status === 'ok') {
      ...
      for (let i = 0; i < queue.length; i++) {
        ...
        if (queue[i].onComplete) {
          // We never unset the output snapshot, and given that this transaction
          // is complete, it should be set
 ...
 * @return {!Path} The rootmost path that was affected by rerunning
 * transactions.
 * @private
 */
...
(Repo.prototype as any).rerunTransactionQueue_ = function(
    queue: Array<Transaction>, path: Path) {
  if (queue.length === 0) {
  ...
  const txnsToRerun =
      queue.filter(function(q) { return q.status === TransactionStatus.RUN; });
  const setsToIgnore =
      txnsToRerun.map(function(q) { return q.currentWriteId; });
  for (let i = 0; i < queue.length; i++) {
    ...
    if (transaction.status === TransactionStatus.NEEDS_ABORT) {
      ...
      if (transaction.retryCount >= (Repo as any).MAX_TRANSACTION_RETRIES_) {
        ...
        events = events.concat(this.serverSyncTree_.ackUserWrite(
            transaction.currentWriteId, true));
      } else {
        // This code reruns a transaction
        const currentNode =
            this.getLatestState_(transaction.path, setsToIgnore);
        transaction.currentInputSnapshot = currentNode;
        const newData = queue[i].update(currentNode.val());
        if (newData !== undefined) {
          validateFirebaseData('transaction failed: Data returned ', newData,
                               transaction.path);
          ...
          if (!hasExplicitPriority) {
            // Keep the old priority if there wasn't a priority explicitly
            // specified.
          ...
          const serverValues = this.generateServerValues();
          const newNodeResolved =
              resolveDeferredValueSnapshot(newDataNode, serverValues);
          ...
          events = events.concat(this.serverSyncTree_.applyUserOverwrite(
              transaction.path, newNodeResolved, transaction.currentWriteId,
    ...
    if (abortTransaction) {
      ...
      // until we're done.
      (function(unwatcher) { setTimeout(unwatcher, Math.floor(0)); })(
          queue[i].unwatcher);

      if (queue[i].onComplete) {
        if (abortReason === 'nodata') {
          const ref = new Reference(this, queue[i].path);
          // We set this field immediately, so it's safe to cast to an actual
          // snapshot
          const lastInput =
              /** @type {!Node} */ (queue[i].currentInputSnapshot);
          const snapshot = new DataSnapshot(lastInput, ref, PRIORITY_INDEX);
          callbacks.push(queue[i].onComplete.bind(null, null, false, snapshot));
        } else {
          callbacks.push(queue[i].onComplete.bind(null, new Error(abortReason),
                                                  false, null));
        }
      }
    }
 ...
 * @private
 */
(Repo.prototype as any).getAncestorTransactionNode_ = function(path: Path):
    Tree<Transaction[]> {
      let front;
...
(Repo.prototype as any).buildTransactionQueue_ = function(
    transactionNode: Tree<Transaction[]>): Array<Transaction> {
...
(Repo.prototype as any).aggregateTransactionQueuesForNode_ = function(
    node: Tree<Transaction[]>, queue: Array<Transaction>) {
  ...
  node.forEachChild(
      (child) => { this.aggregateTransactionQueuesForNode_(child, queue); });
};
...
(Repo.prototype as any).pruneCompletedTransactionsBelowNode_ = function(
  ...
  node.forEachChild(
      (childNode) => { this.pruneCompletedTransactionsBelowNode_(childNode); });
};
...
(Repo.prototype as any).abortTransactions_ = function(path: Path): Path {
  ...
  transactionNode.forEachAncestor(
      (node: Tree<Transaction[]>) => { this.abortTransactionsOnNode_(node); });
    ...
    for (let i = 0; i < queue.length; i++) {
      if (queue[i].status === TransactionStatus.SENT_NEEDS_ABORT) {
        ...
        if (queue[i].onComplete) {
          ...
          callbacks.push(queue[i].onComplete.bind(null, new Error('set'), false,
                                                  snapshot));
        }
      }
    }
