...
export class ViewProcessor {
    ...
    if (writesCache.shadowingWrite(changePath) != null) {
      ...
      if (changePath.isEmpty()) {
        ...
        if (viewCache.getServerCache().isFiltered()) {
          ...
          var completeChildren =
            serverCache instanceof ChildrenNode
              ? serverCache
              : ChildrenNode.EMPTY_NODE;
          var completeEventChildren = writesCache.calcCompleteEventChildren(
            completeChildren
          );
          ...
          var completeNode /** @type {!fb.core.snap.Node} */ = writesCache.calcCompleteEventCache(
        ...
        if (childKey == ".priority") {
          ...
          var updatedPriority = writesCache.calcEventCacheAfterServerOverwrite(
            changePath,
            oldEventNode,
            serverNode
          );
          ...
          if (oldEventSnap.isCompleteForChild(childKey)) {
            serverNode = viewCache.getServerCache().getNode();
            var eventChildUpdate = writesCache.calcEventCacheAfterServerOverwrite(
              changePath,
  ...
  applyServerOverwrite_(
    ...
    var newViewCache = oldViewCache.updateServerSnap(
      newServerCache,
      oldServerSnap.isFullyInitialized() || changePath.isEmpty(),
      serverFilter.filtersNodes()
    );
    var source = new WriteTreeCompleteChildSource(
      writesCache,
      newViewCache,
      optCompleteCache
    );
    ...
    if (changePath.isEmpty()) {
      ...
      if (childKey === ".priority") {
        ...
        if (!oldChild.equals(newChild)) {
          var newEventSnap = this.filter_.updateChild(
            oldEventSnap.getNode(),
            childKey,
            newChild,
            childChangePath,
  ...
  applyUserMerge_(
    ...
    changedChildren.foreach(function(relativePath, childNode) {
      ...
      if (ViewProcessor.cacheHasChild_(viewCache, writePath.getFront())) {
        curViewCache = self.applyUserOverwrite_(
          curViewCache,
          writePath,
          childNode,
          writesCache,
  ...
  applyServerMerge_(
    ...
    viewMergeTree.children.inorderTraversal(function(childKey, childTree) {
      if (serverNode.hasChild(childKey)) {
        ...
        curViewCache = self.applyServerOverwrite_(
          curViewCache,
          new Path(childKey),
          newChild,
          writesCache,
  ...
  revertUserWrite_(
    ...
    if (writesCache.shadowingWrite(path) != null) {
      ...
      if (path.isEmpty() || path.getFront() === ".priority") {
        ...
        var newChild = writesCache.calcCompleteChild(
          childKey,
          viewCache.getServerCache()
        );
        if (
   ...
   * @private
   */
  listenComplete_(viewCache, path, writesCache, serverCache, accumulator) {
    var oldServerNode = viewCache.getServerCache();
    var newViewCache = viewCache.updateServerSnap(
