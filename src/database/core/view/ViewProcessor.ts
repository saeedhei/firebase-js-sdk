...
export class ViewProcessor {
  ...
  applyOperation(oldViewCache: ViewCache, operation: Operation,
                 ...
                 completeCache: Node|null): ProcessorResult {
    const accumulator = new ChildChangeAccumulator();
    let newViewCache, filterServerNode;
    if (operation.type === OperationType.OVERWRITE) {
      const overwrite = operation as Overwrite;
      if (overwrite.source.fromUser) {
        newViewCache = this.applyUserOverwrite_(oldViewCache, overwrite.path,
                                                overwrite.snap, writesCache,
        ...
        newViewCache = this.applyServerOverwrite_(
            oldViewCache, overwrite.path, overwrite.snap, writesCache,
      ...
      if (merge.source.fromUser) {
        newViewCache =
            this.applyUserMerge_(oldViewCache, merge.path, merge.children,
        ...
        newViewCache = this.applyServerMerge_(
            oldViewCache, merge.path, merge.children, writesCache,
            completeCache, filterServerNode, accumulator);
      }
      ...
      if (!ackUserWrite.revert) {
        newViewCache = this.ackUserWrite_(
            oldViewCache, ackUserWrite.path, ackUserWrite.affectedTree,
            writesCache, completeCache, accumulator);
      } else {
        newViewCache =
            this.revertUserWrite_(oldViewCache, ackUserWrite.path, writesCache,
                                  completeCache, accumulator);
      }
    } else if (operation.type === OperationType.LISTEN_COMPLETE) {
      newViewCache = this.listenComplete_(oldViewCache, operation.path,
   ...
   * @private
   */
  private static maybeAddValueEvent_(oldViewCache: ViewCache,
    ...
    if (eventSnap.isFullyInitialized()) {
      ...
      if (accumulator.length > 0 ||
          ...
          (isLeafOrEmpty && !eventSnap.getNode().equals(
                                /** @type {!Node} */ (oldCompleteSnap))) ||
          !eventSnap.getNode().getPriority().equals(
              oldCompleteSnap.getPriority())) {
  ...
  private generateEventCacheAfterServerEvent_(
      viewCache: ViewCache, changePath: Path, writesCache: WriteTreeRef,
    ...
    if (writesCache.shadowingWrite(changePath) != null) {
      ...
      if (changePath.isEmpty()) {
        ...
        if (viewCache.getServerCache().isFiltered()) {
          // We need to special case this, because we need to only apply writes
          // to complete children, or we might end up raising events for
          // incomplete children. If the server data is filtered deep writes
          // cannot be guaranteed to be complete
          const serverCache = viewCache.getCompleteServerSnap();
          const completeChildren = (serverCache instanceof ChildrenNode)
                                       ? serverCache
                                       : ChildrenNode.EMPTY_NODE;
          const completeEventChildren =
              writesCache.calcCompleteEventChildren(completeChildren);
          newEventCache =
              this.filter_.updateFullNode(viewCache.getEventCache().getNode(),
                                          completeEventChildren, accumulator);
        } else {
          ...
          newEventCache = this.filter_.updateFullNode(
              viewCache.getEventCache().getNode(), completeNode, accumulator);
        }
      } else {
        const childKey = changePath.getFront();
        if (childKey == '.priority') {
          assert(changePath.getLength() == 1,
          ...
          const updatedPriority =
              writesCache.calcEventCacheAfterServerOverwrite(
                  changePath, oldEventNode, serverNode);
          if (updatedPriority != null) {
            newEventCache =
                this.filter_.updatePriority(oldEventNode, updatedPriority);
          } else {
          ...
          if (oldEventSnap.isCompleteForChild(childKey)) {
            ...
            if (eventChildUpdate != null) {
              ...
              newEventChild =
                  oldEventSnap.getNode().getImmediateChild(childKey);
            }
          } else {
            newEventChild = writesCache.calcCompleteChild(
                childKey, viewCache.getServerCache());
          }
          if (newEventChild != null) {
            newEventCache = this.filter_.updateChild(
                oldEventSnap.getNode(), childKey, newEventChild,
                childChangePath, source, accumulator);
          } else {
  ...
  applyServerOverwrite_(oldViewCache: ViewCache, changePath: Path,
                        changedSnap: Node, writesCache: WriteTreeRef,
                        completeCache: Node|null, filterServerNode: boolean,
                        accumulator: ChildChangeAccumulator): ViewCache {
    const oldServerSnap = oldViewCache.getServerCache();
    let newServerCache;
    const serverFilter =
        filterServerNode ? this.filter_ : this.filter_.getIndexedFilter();
    if (changePath.isEmpty()) {
      newServerCache = serverFilter.updateFullNode(oldServerSnap.getNode(),
                                                   changedSnap, null);
    } else if (serverFilter.filtersNodes() && !oldServerSnap.isFiltered()) {
      // we want to filter the server node, but we didn't filter the server node
      // yet, so simulate a full update
      const newServerNode =
      ...
      if (childKey == '.priority') {
        newServerCache =
            serverFilter.updatePriority(oldServerSnap.getNode(), newChildNode);
      } else {
        newServerCache = serverFilter.updateChild(
            oldServerSnap.getNode(), childKey, newChildNode, childChangePath,
    ...
    const source = new WriteTreeCompleteChildSource(writesCache, newViewCache,
                                                    completeCache);
  ...
  applyUserOverwrite_(oldViewCache: ViewCache, changePath: Path,
    ...
    const source = new WriteTreeCompleteChildSource(writesCache, oldViewCache,
                                                    completeCache);
    if (changePath.isEmpty()) {
      newEventCache = this.filter_.updateFullNode(
          oldViewCache.getEventCache().getNode(), changedSnap, accumulator);
      newViewCache = oldViewCache.updateEventSnap(newEventCache, true,
                                                  this.filter_.filtersNodes());
    } else {
      const childKey = changePath.getFront();
      if (childKey === '.priority') {
        newEventCache = this.filter_.updatePriority(
            oldViewCache.getEventCache().getNode(), changedSnap);
        newViewCache = oldViewCache.updateEventSnap(
            newEventCache, oldEventSnap.isFullyInitialized(),
        ...
        if (childChangePath.isEmpty()) {
          ...
          if (childNode != null) {
            if (childChangePath.getBack() === '.priority' &&
              ...
              // This is a priority update on an empty node. If this node exists
              // on the server, the server will send down the priority in the
              // update, so ignore for now
              newChild = childNode;
            } else {
        ...
        if (!oldChild.equals(newChild)) {
          const newEventSnap = this.filter_.updateChild(
              oldEventSnap.getNode(), childKey, newChild, childChangePath,
              source, accumulator);
          newViewCache = oldViewCache.updateEventSnap(
              newEventSnap, oldEventSnap.isFullyInitialized(),
              this.filter_.filtersNodes());
        } else {
   ...
   * @private
   */
  private static cacheHasChild_(viewCache: ViewCache,
   ...
   * @private
   */
  private applyUserMerge_(viewCache: ViewCache, path: Path,
                          changedChildren: ImmutableTree<Node>,
                          writesCache: WriteTreeRef, serverCache: Node|null,
                          accumulator: ChildChangeAccumulator): ViewCache {
    ...
    changedChildren.foreach((relativePath, childNode) => {
      ...
      if (ViewProcessor.cacheHasChild_(viewCache, writePath.getFront())) {
        curViewCache =
            this.applyUserOverwrite_(curViewCache, writePath, childNode,
                                     writesCache, serverCache, accumulator);
      }
    });
   ...
   * @private
   */
  private applyServerMerge_(viewCache: ViewCache, path: Path,
                            changedChildren: ImmutableTree<Node>,
                            writesCache: WriteTreeRef, serverCache: Node|null,
    ...
    viewMergeTree.children.inorderTraversal((childKey, childTree) => {
      if (serverNode.hasChild(childKey)) {
        const serverChild =
            viewCache.getServerCache().getNode().getImmediateChild(childKey);
        const newChild = this.applyMerge_(serverChild, childTree);
        curViewCache = this.applyServerOverwrite_(
            curViewCache, new Path(childKey), newChild, writesCache,
   ...
   * @private
   */
  private ackUserWrite_(viewCache: ViewCache, ackPath: Path,
                        affectedTree: ImmutableTree<boolean>,
                        writesCache: WriteTreeRef, completeCache: Node|null,
                        accumulator: ChildChangeAccumulator): ViewCache {
    ...
    if (affectedTree.value != null) {
      ...
      if ((ackPath.isEmpty() && serverCache.isFullyInitialized()) ||
        ...
        return this.applyServerOverwrite_(
            viewCache, ackPath, serverCache.getNode().getChild(ackPath),
        ...
        return this.applyServerMerge_(viewCache, ackPath, changedChildren,
   ...
   * @private
   */
  private listenComplete_(viewCache: ViewCache, path: Path,
                          writesCache: WriteTreeRef,
                          accumulator: ChildChangeAccumulator): ViewCache {
    ...
    return this.generateEventCacheAfterServerEvent_(
        newViewCache, path, writesCache, NO_COMPLETE_CHILD_SOURCE, accumulator);
  }
   ...
   * @private
   */
  private revertUserWrite_(viewCache: ViewCache, path: Path,
                           writesCache: WriteTreeRef,
    ...
    if (writesCache.shadowingWrite(path) != null) {
      ...
      const source = new WriteTreeCompleteChildSource(writesCache, viewCache,
                                                      completeServerCache);
      ...
      if (path.isEmpty() || path.getFront() === '.priority') {
        ...
        if (viewCache.getServerCache().isFullyInitialized()) {
          ...
          newNode = writesCache.calcCompleteEventChildren(serverChildren as
                                                          ChildrenNode);
        }
        newNode = newNode as Node;
        newEventCache =
            this.filter_.updateFullNode(oldEventCache, newNode, accumulator);
      } else {
        const childKey = path.getFront();
        let newChild =
            writesCache.calcCompleteChild(childKey, viewCache.getServerCache());
        if (newChild == null &&
            viewCache.getServerCache().isCompleteForChild(childKey)) {
        ...
        if (newChild != null) {
          newEventCache =
              this.filter_.updateChild(oldEventCache, childKey, newChild,
                                       path.popFront(), source, accumulator);
        } else if (viewCache.getEventCache().getNode().hasChild(childKey)) {
          // No complete child available, delete the existing one, if any
          newEventCache = this.filter_.updateChild(
              oldEventCache, childKey, ChildrenNode.EMPTY_NODE, path.popFront(),
              source, accumulator);
        } else {
        ...
        if (newEventCache.isEmpty() &&
            viewCache.getServerCache().isFullyInitialized()) {
          // We might have reverted all child writes. Maybe the old event was a
          // leaf node
          complete = writesCache.calcCompleteEventCache(
              viewCache.getCompleteServerSnap());
          if (complete.isLeafNode()) {
            newEventCache = this.filter_.updateFullNode(newEventCache, complete,
                                                        accumulator);
          }
        }
      }
      ...
      return viewCache.updateEventSnap(newEventCache, complete,
                                       this.filter_.filtersNodes());
    }
  }
}
