...
export class WriteTree {
   ...
   * @return {!WriteTreeRef}
   */
  childWrites(path: Path): WriteTreeRef { return new WriteTreeRef(path, this); }
   ...
   * @param {boolean=} visible This is set to false by some transactions. It
   * should be excluded from event caches
   */
  addOverwrite(path: Path, snap: Node, writeId: number, visible?: boolean) {
    assert(writeId > this.lastWriteId_,
           'Stacking an older write on top of newer ones');
    if (visible === undefined) {
      visible = true;
    }
    this.allWrites_.push(
        {path : path, snap : snap, writeId : writeId, visible : visible});
   ...
   * true if it may have been visible, meaning views need to reevaluate.
   *
   * @param {!number} writeId
   * @return {boolean} true if the write may have been visible (meaning we'll
   * need to reevaluate / raise events as a result).
   */
  removeWrite(writeId: number): boolean {
    // Note: disabling this check. It could be a transaction that preempted
    // another transaction, and thus was applied out of order.
    // const validClear = revert || this.allWrites_.length === 0 || writeId <=
    // this.allWrites_[0].writeId;  assert(validClear, "Either we don't have this
    // write, or it's the first one in the queue");
    ...
    if (!removedWriteWasVisible) {
      ...
      if (writeToRemove.snap) {
        this.visibleWrites_ =
            this.visibleWrites_.removeWrite(writeToRemove.path);
      } else {
   ...
   * at that path, else null. No server data is considered.
   *
   * @param {!Path} path
   * @return {?Node}
   */
   ...
   * @param {boolean=} includeHiddenWrites Defaults to false, whether or not to
   * layer on writes with visible set to false
   * @return {?Node}
   */
  calcCompleteEventCache(treePath: Path, completeServerCache: Node|null,
    ...
    if (!writeIdsToExclude && !includeHiddenWrites) {
      ...
      if (!includeHiddenWrites && merge.isEmpty()) {
        ...
        // If the server cache is null, and we don't have a complete cache, we
        // need to return null
        if (!includeHiddenWrites && completeServerCache == null &&
          ...
          const mergeAtPath =
              WriteTree.layerTree_(this.allWrites_, filter, treePath);
          const layeredCache = completeServerCache || ChildrenNode.EMPTY_NODE;
   ...
   * @return {!ChildrenNode}
   */
  calcCompleteEventChildren(treePath: Path,
    ...
    if (topLevelSet) {
      if (!topLevelSet.isLeafNode()) {
        ...
        topLevelSet.forEachChild(
            PRIORITY_INDEX, function(childName, childSnap) {
              completeChildren =
                  completeChildren.updateImmediateChild(childName, childSnap);
            });
      }
      ...
      completeServerChildren.forEachChild(
          PRIORITY_INDEX, function(childName, childNode) {
            const node =
                merge.childCompoundWrite(new Path(childName)).apply(childNode);
            completeChildren =
                completeChildren.updateImmediateChild(childName, node);
          });
      // Add any complete children we have from the set
      merge.getCompleteChildren().forEach(function(namedNode) {
        completeChildren = completeChildren.updateImmediateChild(
            namedNode.name, namedNode.node);
      });
      return completeChildren;
    } else {
      // We don't have anything to layer on top of. Layer on any children we
      // have Note that we can return an empty snap if we have a defined delete
      const merge = this.visibleWrites_.childCompoundWrite(treePath);
      merge.getCompleteChildren().forEach(function(namedNode) {
        completeChildren = completeChildren.updateImmediateChild(
            namedNode.name, namedNode.node);
   ...
   * @return {?Node}
   */
  calcEventCacheAfterServerOverwrite(treePath: Path, childPath: Path,
   ...
   * writes or null if there is no complete child for this ChildKey.
   *
   * @param {!Path} treePath
   ...
   * @return {?Node}
   */
  calcCompleteChild(treePath: Path, childKey: string,
                    existingServerSnap: CacheNode): Node|null {
   ...
   */
  calcIndexedSlice(treePath: Path, completeServerData: Node|null,
                   startPost: NamedNode, count: number, reverse: boolean,
                   index: Index): NamedNode[] {
    let toIterate: Node;
    const merge = this.visibleWrites_.childCompoundWrite(treePath);
    ...
    if (!toIterate.isEmpty() && !toIterate.isLeafNode()) {
      ...
      const iter =
          reverse
              ? (toIterate as ChildrenNode)
                    .getReverseIteratorFrom(startPost, index)
  ...
  private recordContainsPath_(writeRecord: WriteRecord, path: Path): boolean {
    if (writeRecord.snap) {
      ...
      return !!findKey(
          writeRecord.children, function(childSnap: Node, childName: string) {
  ...
  private resetTree_() {
    this.visibleWrites_ = WriteTree.layerTree_(
        this.allWrites_, WriteTree.DefaultFilter_, Path.Empty);
   ...
   * @private
   */
  private static DefaultFilter_(write: WriteRecord) { return write.visible; }

  /**
   * Static method. Given an array of WriteRecords, a filter for which ones to
   * include, and a path, construct the tree of event data at that path.
   *
   ...
   * @private
   */
  private static layerTree_(writes: WriteRecord[],
                            filter: (w: WriteRecord) => boolean,
                            treeRoot: Path): CompoundWrite {
        ...
        if (write.snap) {
          if (treeRoot.contains(writePath)) {
            ...
            compoundWrite = compoundWrite.addWrite(
                Path.Empty, write.snap.getChild(relativePath));
          } else {
          ...
          if (treeRoot.contains(writePath)) {
            relativePath = Path.relativePath(treeRoot, writePath);
            compoundWrite =
                compoundWrite.addWrites(relativePath, write.children);
          } else if (writePath.contains(treeRoot)) {
            relativePath = Path.relativePath(writePath, treeRoot);
            if (relativePath.isEmpty()) {
              compoundWrite =
                  compoundWrite.addWrites(Path.Empty, write.children);
            } else {
...
export class WriteTreeRef {
   ...
   * writeTree_ while exposing a simpler interface to callers.
   *
   * @type {!Path}
   * @private
   * @const
  ...
  /**
   * * A reference to the actual tree of write data. All methods are
  ...
  /**
   * If possible, returns a complete event cache, using the underlying server
   * data if possible. In addition, can be used to get a cache that includes
   * hidden writes, and excludes arbitrary writes. Note that customizing the
   * returned node can lead to a more expensive calculation.
   *
   * @param {?Node} completeServerCache
   ...
   * @return {?Node}
   */
  calcCompleteEventCache(completeServerCache: Node|null,
                         writeIdsToExclude?: number[],
                         includeHiddenWrites?: boolean): Node|null {
    return this.writeTree_.calcCompleteEventCache(
        this.treePath_, completeServerCache, writeIdsToExclude,
        includeHiddenWrites);
  }

  /**
   * If possible, returns a children node containing all of the complete
   ...
   * @return {!ChildrenNode}
   */
  calcCompleteEventChildren(completeServerChildren: ChildrenNode|
                            null): ChildrenNode {
    return this.writeTree_.calcCompleteEventChildren(
               this.treePath_, completeServerChildren) as ChildrenNode;
  }

  /**
   * Given that either the underlying server data has updated or the outstanding
   * writes have updated, determine what, if anything, needs to be applied to
   * the event cache.
   *
   * Possibilities:
   *
   ...
   *
   * Either existingEventSnap or existingServerSnap must exist, this is
   * validated via an assert
   *
   * @param {!Path} path
   * @param {?Node} existingEventSnap
   * @param {?Node} existingServerSnap
  ...
  calcEventCacheAfterServerOverwrite(path: Path, existingEventSnap: Node|null,
    ...
    return this.writeTree_.calcEventCacheAfterServerOverwrite(
        this.treePath_, path, existingEventSnap, existingServerSnap);
  }
   ...
   * @return {!Array.<!NamedNode>}
   */
  calcIndexedSlice(completeServerData: Node|null, startPost: NamedNode,
                   count: number, reverse: boolean, index: Index): NamedNode[] {
    return this.writeTree_.calcIndexedSlice(this.treePath_, completeServerData,
                                            startPost, count, reverse, index);
  }
   ...
   * @return {?Node}
   */
  calcCompleteChild(childKey: string, existingServerCache: CacheNode): Node
      |null {
    return this.writeTree_.calcCompleteChild(this.treePath_, childKey,
                                             existingServerCache);
  }
