 ...
 * @param {?string|?number=} priority Optional priority to use.  This will be
 * ignored if the passed JSON contains a .priority property.
 * @return {!Node}
 */
export function nodeFromJSON(json: any|null,
  ...
  assert(priority === null || typeof priority === 'string' ||
  ...
  if (!(json instanceof Array) && USE_HINZE) {
    ...
    forEach(hinzeJsonObj, (key: string, child: any) => {
      if (typeof key !== 'string' ||
          key.substring(0, 1) !== '.') { // Ignore metadata nodes
    ...
    const childSet = buildChildSet(children, NAME_ONLY_COMPARATOR,
                                   ...
                                   NAME_COMPARATOR) as SortedMap<string, Node>;
    if (childrenHavePriority) {
      const sortedChildSet =
          buildChildSet(children, PRIORITY_INDEX.getCompare());
      return new ChildrenNode(childSet, nodeFromJSON(priority),
                              new IndexMap({'.priority' : sortedChildSet},
