...
export class Sha1 extends Hash {
  ...
  compress_(buf, opt_offset?) {
    ...
    if (typeof buf === 'string') {
      for (var i = 0; i < 16; i++) {
        // TODO(user): [bug 8140122] Recent versions of Safari for Mac OS and
        // iOS have a bug that turns the post-increment ++ operator into
        // pre-increment during JIT compilation.  We have code that depends
        // heavily on SHA-1 for correctness and which is affected by this bug,
        // so I've removed all uses of post-increment ++ in which the result
        // value is used.  We can revert this change once the Safari bug
