...
export default class VapidDetailsModel extends DBInterface {
  ...
  constructor() { super(VapidDetailsModel.dbName, DB_VERSION); }

  static get dbName() { return 'fcm_vapid_details_db'; }
   ...
   */
  onDBUpgrade(db) {
    db.createObjectStore(FCM_VAPID_OBJ_STORE, {keyPath : 'swScope'});
  }
  ...
  saveVapidDetails(swScope, vapidKey) {
    ...
    const details = {'swScope' : swScope, 'vapidKey' : vapidKey};
          ...
          request.onsuccess = event => {
            if ((<IDBRequest>event.target).result === 0) {
              reject(this.errorFactory_.create(
                  Errors.codes.FAILED_DELETE_VAPID_KEY));
              return;
            }
